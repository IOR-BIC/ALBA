...
Module:    $RCSfile: mVueGlCanvas.h,v $
Language:  C++
...
#ifndef __mVueGlCanvas_h__
#define __mVueGlCanvas_h__
...
//--------------------------------------------------------------------------------
class mVueGlCanvas: public wxGLCanvas, public osgGA::GUIActionAdapter
//--------------------------------------------------------------------------------
{
public:
    // EventQueue
    typedef std::vector< osg::ref_ptr<wxWinEventAdapter> > EventQueueType;
...
public:
    mVueGlCanvas(wxWindow *parent,const wxWindowID id = -1, const wxPoint &pos = wxDefaultPosition, const wxSize &size = wxDefaultSize,long style = 0);
   ~mVueGlCanvas(void);
...
protected:
    ...
    void EventQueue(EventQueueType& queue);
        // return event queue
    void AddEvent(osg::ref_ptr<wxWinEventAdapter> ea);
...
private:
    // WDR: member variable declarations for mVueGlCanvas
    wxTimer *m_Timer;
        // timer
