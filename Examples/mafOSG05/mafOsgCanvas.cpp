...
//------------------------------------------------------------------------------
{
  osg::ref_ptr<osg::Node> load_try = osgDB::readNodeFile(filename);
  if( load_try.get() == NULL ) return false;

  osgUtil::Optimizer optimizer;
  optimizer.optimize(load_try.get());

  //while (m_Root->getNumChildren() > 0) 
  //    m_Root->removeChild(m_Root->getChild(0));

  if(m_loadedModel.get())
    m_Root->removeChild(m_loadedModel.get());
  
  m_loadedModel = load_try.get();
  
  m_Root->addChild(m_loadedModel.get());
