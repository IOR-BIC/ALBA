...
//----------------------------------------------------------------------------
{
  ...
  m_manip = new mafOsgTerrainManipulator();
  m_manip->setNode(m_canvas->GetRoot());
  m_manip->setGizmoParent(m_canvas->GetRoot());

  // connect manipulator to the Viewer
  osgProducer::Viewer *viewer = m_canvas->GetViewer(); 
  assert(viewer);
  unsigned int pos = viewer->addCameraManipulator(m_manip.get());
  viewer->selectCameraManipulator(pos);

  m_manip_gui = new mafOsgTerrainManipulatorGui(this,m_manip.get());

  m_sky = new mafOsgSky(viewer, m_manip.get());
...
//----------------------------------------------------------------------------
{
  ...
  m_settings_panel->Put(m_manip_gui->GetGui());
...
//----------------------------------------------------------------------------
{
  ...
  m_sky = new mafOsgSky(m_canvas->GetViewer(), m_manip.get());
