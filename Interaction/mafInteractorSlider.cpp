/*=========================================================================
Program:   ALBA
Module:    mafInteractorSlider.cpp
Language:  C++
...
//----------------------------------------------------------------------------
// NOTE: Every CPP file in the ALBA must include "mafDefines.h" as first.
...
//------------------------------------------------------------------------------
mafCxxTypeMacro(mafInteractorSlider)

// EVENT_TABLE
BEGIN_EVENT_TABLE(mafInteractorSlider, wxFrame)
EVT_TIMER(ID_TIMER, mafInteractorSlider::OnTimer)
END_EVENT_TABLE()

//----------------------------------------------------------------------------
mafInteractorSlider::mafInteractorSlider()
{
	m_TestMode = false;
...
//----------------------------------------------------------------------------
mafInteractorSlider::mafInteractorSlider(mafView *view, int orientation, double value, double minValue, double maxValue) : mafInteractorSlider()
{
...
//----------------------------------------------------------------------------
mafInteractorSlider::mafInteractorSlider(mafView *view, int orientation, double value, double minValue, double maxValue, int x, int y, int lenght) : mafInteractorSlider()
{
	SetRendererByView(view);
...
// EVENTS
//----------------------------------------------------------------------------
void mafInteractorSlider::OnEvent(mafEventBase *event)
{
...
//----------------------------------------------------------------------------
void mafInteractorSlider::OnTimer(wxTimerEvent& event)
{
	double timeStep = 1.0;
	m_Time += timeStep;
...
// MOUSE EVENTS
//----------------------------------------------------------------------------
void mafInteractorSlider::OnLeftButtonDown(mafEventInteraction *e)
{
...
//----------------------------------------------------------------------------
void mafInteractorSlider::OnLeftButtonUp(mafEventInteraction *e)
{
	...
	if (m_ParallelView && m_IsInBound)
	{
		...
		if (m_timer.IsRunning())
		{
			//mafLogMessage("--------- Stop Timer ---------");
			m_timer.Stop();
		}
	}
}
//----------------------------------------------------------------------------
void mafInteractorSlider::OnMove(mafEventInteraction *e)
{
	Superclass::OnMove(e);
...
//----------------------------------------------------------------------------
void mafInteractorSlider::UpdatePadPosition(double pos)
{
	m_PadPosition = pos;
	...
	mafEventMacro(mafEvent(this, ID_VALUE_CHANGED, m_Value));
}
//----------------------------------------------------------------------------
void mafInteractorSlider::UpdateSlider()
{
...
//----------------------------------------------------------------------------
double mafInteractorSlider::ValueToPosition(double value)
{
...
//----------------------------------------------------------------------------
bool mafInteractorSlider::IsInBound(double *pos)
{
	m_IsInBound = false;
...
//----------------------------------------------------------------------------
void mafInteractorSlider::SetRendererByView(mafView *view)
{
	m_View = view;
...
//----------------------------------------------------------------------------
void mafInteractorSlider::SetSteps(int steps)
{
	m_Steps = steps;
...
//----------------------------------------------------------------------------
void mafInteractorSlider::SetRange(double min, double max)
{
	m_MinValue = min;
	m_MaxValue = max;
...
//----------------------------------------------------------------------------
void mafInteractorSlider::SetValue(double value)
{
	m_Value = value;
...
//----------------------------------------------------------------------------
void mafInteractorSlider::ShowSlider(bool show)
{
	if (m_Renderer == NULL)
		return;
