...
// mafGUILutEditor
//----------------------------------------------------------------------------
BEGIN_EVENT_TABLE(mafGUILutEditor,wxPanel)
  //EVT_COMBOBOX(ID_PRESET, mafGUILutEditor::OnComboSelection)
END_EVENT_TABLE()

//----------------------------------------------------------------------------
mafGUILutEditor::mafGUILutEditor(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
...
//----------------------------------------------------------------------------
{
  ...
  m_UserLutLibraryDir = (mafGetAppDataDirectory()+"\\UserDefinedLookupTables\\").c_str();
  ...
  m_PresetCombo->SetValidator( mafGUIValidator(this,ID_PRESET,m_PresetCombo,&m_Preset) );
	...
	text->SetValidator( mafGUIValidator(this,ID_NUMENTRY,text,&m_NumEntry,1,256) );
	sz = new wxBoxSizer(wxHORIZONTAL);
	...
	text->SetValidator( mafGUIValidator(this,ID_VALUE_RANGE,text,&m_ValueRange[0], -1e200, 1e200, -1 ) ); //SIL: 17-01-06 - removed number of digits -- introduce errors on big numbers
	text2 = new wxTextCtrl  (this, -1, ""   ,       dp, wxSize(DW/2,LH), wxNO_BORDER);
	text2->SetValidator( mafGUIValidator(this,ID_VALUE_RANGE,text2,&m_ValueRange[1] ,-1e200, 1e200, -1 ) ); //SIL: 17-01-06 - removed number of digits -- introduce errors on big numbers
  sz = new wxBoxSizer(wxHORIZONTAL);
	sz->Add( lab,   0, wxRIGHT, LM);
	sz->Add( text,  0, wxRIGHT, HM);
  ...
  textNewUserLUTName->SetValidator( mafGUIValidator(this,ID_NEW_USER_LUT_NAME,textNewUserLUTName,&m_NewUserLutName) );
  ...
  m_UserPresetCombo->SetValidator( mafGUIValidator(this,ID_USER_PRESET,m_UserPresetCombo,&m_UserPreset) );
  ...
  m_LutSwatch = new mafGUILutSwatch(this, -1, dp,wxSize(286,16));
...
// 	sizer->Add( lab, 0, wxALL, M);
  ...
  m_LutWidget = new mafGUILutWidget(this,ID_LW);//,dp,wxSize(300,300));
  ...
  sizer->Add(m_LutWidget, 0, wxALL, M);
	...
	lab->SetValidator( mafGUIValidator(this,-1,lab,&m_Info) );
  sizer->Add(lab, 0, wxALL, M);
...
// 	sizer->Add( lab, 0, wxALL, M);
  ...
  m_ColorWidget = new mafGUIColorWidget(this,ID_CW);//,dp,wxSize(300,300));
...
//----------------------------------------------------------------------------
mafGUILutEditor::~mafGUILutEditor()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
/*void mafGUILutEditor::OnComboSelection(wxCommandEvent &event)
//----------------------------------------------------------------------------
{
	UpdateLut();
  //TransferDataToWindow();
...
//----------------------------------------------------------------------------
void mafGUILutEditor::UpdateInfo()
//----------------------------------------------------------------------------
{
  int min, max;
...
//----------------------------------------------------------------------------
void mafGUILutEditor::SetLut(vtkLookupTable *lut)
//----------------------------------------------------------------------------
{
  m_ExternalLut = lut;
...
//----------------------------------------------------------------------------
void mafGUILutEditor::CopyLut(vtkLookupTable *from, vtkLookupTable *to)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
  ...
  mafGUILutEditor *led = new mafGUILutEditor(&dlg, id);
...
void mafGUILutEditor::UpdateWidgetsOnLutChange()
{
  m_NumEntry = m_Lut->GetNumberOfTableValues();
  m_ValueRange[0] = m_Lut->GetRange()[0];
  m_ValueRange[1] = m_Lut->GetRange()[1];
