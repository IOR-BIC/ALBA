/*=========================================================================
 ...
 Program: MAF2
 Module: mafGUILutHistogramSwatch
...
// mafGUILutSwatch
//----------------------------------------------------------------------------
BEGIN_EVENT_TABLE(mafGUILutHistogramSwatch,wxPanel)
  EVT_PAINT(mafGUILutHistogramSwatch::OnPaint)
  EVT_ERASE_BACKGROUND(mafGUILutHistogramSwatch::OnEraseBackground) 

  EVT_LEFT_DOWN(mafGUILutHistogramSwatch::OnLeftMouseButtonDown)
  EVT_LEFT_UP(mafGUILutHistogramSwatch::OnLeftMouseButtonUp)
  EVT_MOTION(mafGUILutHistogramSwatch::OnMouseMotion)
  EVT_SIZE(mafGUILutHistogramSwatch::OnSize)
 
END_EVENT_TABLE()

//----------------------------------------------------------------------------
mafGUILutHistogramSwatch::mafGUILutHistogramSwatch(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
...
mafGUILutHistogramSwatch::mafGUILutHistogramSwatch(mafGUI *parent, wxWindowID id, wxString name, vtkDataSet *dataSet, mmaVolumeMaterial *material, wxSize size, bool showText)
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::OnPaint(wxPaintEvent &event)
//----------------------------------------------------------------------------
{
  wxPaintDC pdc(this);
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::OnLeftMouseButtonDown(wxMouseEvent &event)
//----------------------------------------------------------------------------
{
  if( m_Lut && m_Editable  )
  {
    ...
    mafGUILutHistogramEditor::ShowLutHistogramDialog(m_DataSet,m_Material,"Histogram Lut Editor",m_Listener,  GetId());
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::OnLeftMouseButtonUp(wxMouseEvent &event)
//----------------------------------------------------------------------------
{
  //no left mouse button functionality now.
}
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::OnMouseMotion(wxMouseEvent &event)
//----------------------------------------------------------------------------
{
  int x = event.m_x;
  int y = event.m_y;
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::OnSize(wxSizeEvent &event)
//----------------------------------------------------------------------------
{
  m_Material->ApplyGammaCorrection(4);
  m_Material->UpdateProp();
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::SetMaterial(mmaVolumeMaterial *material)
//----------------------------------------------------------------------------
{
  //update lut pointer
  m_Material = material;
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::SetDataSet(vtkDataSet *dataSet)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::ShowThreshold(bool b) 
 //----------------------------------------------------------------------------
{
  m_ShowThreshold =b;
  Modified();
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::Modified()
//----------------------------------------------------------------------------
{
  Update();
  Refresh();
...
//----------------------------------------------------------------------------
int mafGUILutHistogramSwatch::GetLutIndexByPos(float fullWidth, float x)
//----------------------------------------------------------------------------
{
  float i;
...
//----------------------------------------------------------------------------
void mafGUILutHistogramSwatch::Update()
//----------------------------------------------------------------------------
{
  int w = GetSize().GetWidth();   
  int h = GetSize().GetHeight();   
