...
//----------------------------------------------------------------------------
void mafGUILutSwatch::SetEditable(bool b)
{
	m_Editable = b;
}
...
//----------------------------------------------------------------------------
{
	...
	for (y = 0; y<h; y++)
	{
		...
		{
			float i = (num * x) / w;
			mafColor col = (m_Enabled && m_Lut) ? mafColor(m_Lut->GetTableValue(i)) : mafColor(225, 225, 225);
...
//----------------------------------------------------------------------------
bool mafGUILutSwatch::Enable(bool enable /*= true*/)
{
	m_Enabled = enable;
	Update();
	Refresh();
