...
//----------------------------------------------------------------------------
void mafGUI::Bool(int id, mafString label, int* var, int flag, mafString tooltip)   
{
...
//----------------------------------------------------------------------------
mafGUIFloatSlider *mafGUI::FloatSlider(int id, wxString label, double *var, double min, double max, wxSize size, wxString tooltip, bool textBoxEnable)
{
...
//----------------------------------------------------------------------------
mafGUIFloatSlider *mafGUI::FloatExpandedSlider(int id, wxString label, double *var, double min, double max, wxSize size, wxString tooltip, bool textBoxEnable)
{
	//<*> Si puo Chiamare Slider lo stesso 
...
//----------------------------------------------------------------------------
void mafGUI::DoubleUpDown(int labelID, int firstID, int secondID, mafString label, double *var, double min /*= MINDOUBLE*/, double max /*= MAXDOUBLE*/, int decimal_digit /*= -1*/, mafString tooltip /*= ""*/, bool labelAlwaysEnable /*= false*/)
{
	// Double entry
	int w_id;
	wxBoxSizer *sizer = new wxBoxSizer(wxHORIZONTAL);
	...
	int w_idFirst = GetWidgetId(firstID);
	mafGUIButton    *b1 = new mafGUIButton(this, w_idFirst, "-", dp, wxSize(w, BH));
	b1->SetValidator(mafGUIValidator(this, w_idFirst, b1));
	b1->SetFont(m_Font);

	int w_idSecond = GetWidgetId(secondID);
	mafGUIButton    *b2 = new mafGUIButton(this, w_idSecond, "+", dp, wxSize(w, BH));
	b2->SetValidator(mafGUIValidator(this, w_idSecond, b2));
	b2->SetFont(m_Font);
...
//----------------------------------------------------------------------------
void mafGUI::ImageButton(int id, const char* label, wxBitmap bitmap, mafString tooltip)
{
	int w_id = GetWidgetId(id);
...
//----------------------------------------------------------------------------
wxGrid *mafGUI::Grid(int id, wxString label, int height, int row, int cols, wxString tooltip)
{
...
//----------------------------------------------------------------------------
{
	...
	OnEvent(&mafEvent(this, event.GetId(),index));
}
//----------------------------------------------------------------------------
