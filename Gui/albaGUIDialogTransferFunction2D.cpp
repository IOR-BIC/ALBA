...
BEGIN_EVENT_TABLE(albaGUIDialogTransferFunction2D, wxDialog)
  EVT_IDLE(albaGUIDialogTransferFunction2D::OnIdle)
  EVT_MOTION(albaGUIDialogTransferFunction2D::OnMotion)
  EVT_SIZE(albaGUIDialogTransferFunction2D::OnSize)
  EVT_COMMAND_RANGE(MINID, MAXID, wxEVT_COMMAND_LISTBOX_SELECTED, albaGUIDialogTransferFunction2D::OnCommand)
  EVT_COMMAND_RANGE(MINID, MAXID, wxEVT_COMMAND_SLIDER_UPDATED,   albaGUIDialogTransferFunction2D::OnCommand)
  EVT_COMMAND_RANGE(MINID, MAXID, wxEVT_COMMAND_CHOICE_SELECTED,  albaGUIDialogTransferFunction2D::OnCommand)
END_EVENT_TABLE()
...
//----------------------------------------------------------------------------
{
  ...
  if (this->m_VolumeProperty->GetTransferFunction2D() == NULL) 
	{
    this->m_TransferFunction = vtkALBATransferFunction2D::New();
...
//----------------------------------------------------------------------------
{
  ...
  this->m_SliceRwi = new albaRWIBase(previewPage, -1);
  ...
  // windowing slider
  this->m_WindowingSlider = new albaGUIRangeSlider(previewPage, ID_WINDOWING_SLIDER, wxDefaultPosition, wxSize(-1, 30), wxNO_BORDER);
  ...
  this->m_Rwi3D = new albaRWIBase(previewPage, -1);
  ...
  this->m_GraphRwi = new albaRWIBase(this, -1);
  ...
  // add / remove buttons
  albaGUIButton  *b1, *b2;
  b1 = new albaGUIButton(this, ID_TF_ADD,    "add",wxDefaultPosition, wxSize(40, 20));
  b1->SetValidator(albaGUIValidator(this, ID_TF_ADD, b1));
  b2 = new albaGUIButton(this, ID_TF_REMOVE, "remove", wxDefaultPosition, wxSize(40, 20));
  b2->SetValidator(albaGUIValidator(this, ID_TF_REMOVE, b2));
  ...
  wxTextCtrl  *text = new wxTextCtrl(this, ID_TF_NAME, this->m_WidgetName, wxDefaultPosition, wxSize(-1, 20), wxNO_BORDER);
	text->SetValidator(albaGUIValidator(this, ID_TF_NAME, text, &this->m_WidgetName));
  ...
  albaGUIFloatSlider *slider = new albaGUIFloatSlider(this, ID_TF_OPACITY, 0, 0, 1, wxDefaultPosition, wxSize(40, 20));
  text->SetValidator(albaGUIValidator(this, ID_TF_OPACITY, text, &this->m_Widget.Opacity, slider, 0.f, 1.f));
	slider->SetValidator(albaGUIValidator(this, ID_TF_OPACITY, slider, &this->m_Widget.Opacity, text));
  ...
  slider = new albaGUIFloatSlider(this, ID_TF_DIFFUSE, 0, 0, 1, wxDefaultPosition, wxSize(40, 20));
  text->SetValidator(albaGUIValidator(this, ID_TF_DIFFUSE, text, &this->m_Widget.Diffuse, slider, 0.f, 1.f));
	slider->SetValidator(albaGUIValidator(this, ID_TF_DIFFUSE, slider, &this->m_Widget.Diffuse, text));
  ...
  this->m_ValueSlider = new albaGUIRangeSlider(this, ID_TF_VALUE, wxDefaultPosition, wxSize(-1, 30), wxNO_BORDER);
  ...
  for (j = 0; j < 3; j++) 
    ...
    text->SetValidator(albaGUIValidator(this, ID_TF_VALUE_0 + j, text, this->m_Widget.Range[0] + (j == 0 ? 0 : (j == 1 ? 2 : 1)), -99999, 99999, 0));
  ...
  this->m_GradientSlider = new albaGUIRangeSlider(this, ID_TF_GRADIENT, wxDefaultPosition, wxSize(-1, 30), wxNO_BORDER);
  ...
  for (j = 0; j < 3; j++) 
    ...
    text->SetValidator(albaGUIValidator(this, ID_TF_GRADIENT_0 + j, text, this->m_Widget.Range[1] + (j == 0 ? 0 : (j == 1 ? 2 : 1)), -99999, 99999, 0));
    sizer->Add(text, 1, wxEXPAND | wxALIGN_LEFT | wxLEFT | wxRIGHT, 5);
  }
  ...
  // library
//  b1 = new albaGUIButton(this, ID_TF_LOAD, "load", wxDefaultPosition, wxSize(-1, 20));
//  b1->SetValidator(albaGUIValidator(this, ID_TF_LOAD, b1));
//  b2 = new albaGUIButton(this, ID_TF_SAVE, "save",wxDefaultPosition, wxSize(-1, 20));
//  b2->SetValidator(albaGUIValidator(this, ID_TF_SAVE, b2));
...
//  rightPaneL->Add(new wxStaticText(this, -1, "", wxDefaultPosition, wxSize(-1, 20)), wxEXPAND, 0);
  ...
  // ok, cancel
  b1 = new albaGUIButton(this, wxOK,    "ok", wxDefaultPosition, wxSize(-1, 20));
  b1->SetValidator(albaGUIValidator(this, wxOK, b1));
  b2 = new albaGUIButton(this, wxCANCEL,    "cancel",wxDefaultPosition, wxSize(-1, 20));
...
//----------------------------------------------------------------------------
VTK_THREAD_RETURN_TYPE albaGUIDialogTransferFunction2D::CreatePipe(void *argDialog) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaGUIDialogTransferFunction2D::OnEvent(albaEventBase *alba_event) 
//----------------------------------------------------------------------------
{
  if (this->m_SlicePipeStatus < PipeReady)
    return;
...
/*      if(m_Vme->GetTagArray()->FindTag("VOLUME_TRANSFER_FUNCTION") == -1)
      ...
      vtkALBATransferFunction2D *newf = vtkALBATransferFunction2D::New();
			...
			if (newf->LoadFromString(string.ToAscii()))
    ...
    case ID_TF_VALUE_1:
    case ID_TF_VALUE_2:
      if (alba_event->GetId() == ID_TF_VALUE) 
			{
    ...
    case ID_TF_GRADIENT_1:
    case ID_TF_GRADIENT_2:
      if (alba_event->GetId() == ID_TF_GRADIENT) 
			{
