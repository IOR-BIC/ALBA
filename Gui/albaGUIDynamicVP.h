/*========================================================================= 
  ...
  Module: $RCSfile: albaGUIDynamicVP.h,v $ 
  Language: C++ 
  Date: $Date: 2012-04-06 08:28:41 $ 
...
#ifndef __albaGUIDynamicVP_H__
#define __albaGUIDynamicVP_H__
...
class albaView;
class albaGUI;
class albaGUIScrolledPanel;
class wxStaticLine;

//----------------------------------------------------------------------------
/**
 albaGUIDynamicVP is a wxPanel with a set of widget to handle dynamic 
...
listener is notified by albaEvent with ID equal to the specified one (given
in ctor) and with ID_CREATE_VP or ID_CLOSE_VP argument (see
albaEvent::GetArg). albaGUIDynamicVP can be configured so it does not contain
...
//----------------------------------------------------------------------------
class ALBA_EXPORT albaGUIDynamicVP: public albaGUIPanel, public albaObserver
{
public:  
...
protected:  
  ...
  albaObserver* m_Listener;        ///<object that is notified when something changes (notified by ID)
  ...
  albaPipe* m_VPipe;             ///<this is the current visual pipe  
  albaSceneNode* m_SceneNode;    ///<reference to the scene node associated with this pipe
  albaString m_Name;             ///<user name associated with this panel
...
public:
  /** constructor */
  albaGUIDynamicVP(wxWindow* parent, wxWindowID id, 
  ...
  inline albaObserver* GetListener(){
    return m_Listener;
  };

  /** Specifies the listener that will be notified when any control changes. */
  inline void SetListener(albaObserver *Listener){
    m_Listener = Listener;
  };
  ...
  inline albaSceneNode* GetSceneNode() {
    return m_SceneNode;
  }
  ...
  inline albaPipe* GetCurrentVisualPipe() {
    return m_VPipe;
  }
