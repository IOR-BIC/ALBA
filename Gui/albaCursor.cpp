...
void albaCursor::SetCursor(wxWindow *window, const int cursor)
{
	...
	if (cursor < wxCURSOR_MAX)
	{
		// Forward to wxWidgets API to change the global cursor
		window->SetCursor(wxCursor((wxStockCursor)cursor));
	}
	else if (cursor >= wxCURSOR_MAX && cursor < wxCURSOR_MAX + GLO_Cursors.size())
	{
		// Custom cursor handling: if the cursor value corresponds to a custom cursor, set it.
		window->SetCursor(*GLO_Cursors[cursor - wxCURSOR_MAX]);
	}
}
