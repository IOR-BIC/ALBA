/*=========================================================================

 Program: ALBA (Agile Library for Biomedical Applications)
 Module: albaInteractorDICOMImporterTest
 Authors: Stefano Perticoni
 
 Copyright (c) BIC
 All rights reserved. See Copyright.txt or


 This software is distributed WITHOUT ANY WARRANTY; without even
 the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
 PURPOSE.  See the above copyright notice for more information.

=========================================================================*/

#include "albaDefines.h" 
//----------------------------------------------------------------------------
// NOTE: Every CPP file in the ALBA must include "albaDefines.h" as first.
// This force to include Window,wxWidgets and VTK exactly in this order.
// Failing in doing this will result in a run-time error saying:
// "Failure#0: The value of ESP was not properly saved across a function call"
//----------------------------------------------------------------------------

#include "albaInteractorDICOMImporterTest.h"

#include <cppunit/config/SourcePrefix.h>
#include <iostream>
#include "albaInteractorDICOMImporter.h"
#include "albaEventInteraction.h"
#include "albaDeviceButtonsPadMouse.h"

void albaInteractorDICOMImporterTest::TestFixture()
{
	
}


void albaInteractorDICOMImporterTest::TestConstructorDestructor()
{
	albaInteractorDICOMImporter *dicomImporterInteractor = albaInteractorDICOMImporter::New();
	dicomImporterInteractor->Delete();
}

void albaInteractorDICOMImporterTest::TestStartInteraction()
{
	albaDeviceButtonsPadMouse *buttonsPadMouse = albaDeviceButtonsPadMouse::New();
	CPPUNIT_ASSERT(buttonsPadMouse);

	// smoke test (should not crash)
	albaInteractorDICOMImporter *dicomImporterInteractor = albaInteractorDICOMImporter::New();
	dicomImporterInteractor->StartInteraction(buttonsPadMouse);
	dicomImporterInteractor->Delete();

	buttonsPadMouse->Delete();
}


void albaInteractorDICOMImporterTest::TestOnMouseMove()
{
  albaInteractorDICOMImporter *dicomImporterInteractor = albaInteractorDICOMImporter::New();
  
  // smoke test (should not crash)
  dicomImporterInteractor->OnMouseMove();
  dicomImporterInteractor->Delete();
}


void albaInteractorDICOMImporterTest::TestOnLeftButtonDown()
{
  albaInteractorDICOMImporter *dicomImporterInteractor = albaInteractorDICOMImporter::New();
  
	albaEventInteraction e;
  // smoke test (should not crash)
  dicomImporterInteractor->OnLeftButtonDown(&e);
  dicomImporterInteractor->Delete();
}

void albaInteractorDICOMImporterTest::TestOnLeftButtonUp()
{
	albaInteractorDICOMImporter *dicomImporterInteractor = albaInteractorDICOMImporter::New();

	// smoke test (should not crash)
	dicomImporterInteractor->OnLeftButtonUp();
	dicomImporterInteractor->Delete();
}

