...
/** mafTransform - class for multi frame homogeneous transformations.
  ...
  be expressed as a mafTrasformBase.
  ...
  - Aggiorna il test della mafTransform per testare le funzioni di conversione degli angoli
*/
class MAF_EXPORT mafTransform : public mafTrasformBase
{
 public:
  //------------------------------------------------------------------------------
  // Events
  ...
  static mafTransform *New();
  vtkTypeMacro(mafTransform,mafTrasformBase);
  ...
  virtual void SetInput(mafTrasformBase *);
  vtkGetObjectMacro(Input,mafTrasformBase);
  ...
  void SetInputFrame(mafTrasformBase *frame);
  mafTrasformBase *GetInputFrame() {return this->InputFrame;}
  ...
  void SetTargetFrame(mafTrasformBase *frame);
  mafTrasformBase *GetTargetFrame() {return this->TargetFrame;}
  ...
  static void RotateX(vtkMatrix4x4 *matrix,double angle,int premultiply) { mafTransform::RotateWXYZ(matrix,matrix,angle, 1, 0, 0,premultiply); };
  static void RotateY(vtkMatrix4x4 *matrix,double angle,int premultiply) { mafTransform::RotateWXYZ(matrix,matrix,angle, 0, 1, 0,premultiply); };
  static void RotateZ(vtkMatrix4x4 *matrix,double angle,int premultiply) { mafTransform::RotateWXYZ(matrix,matrix,angle, 0, 0, 1,premultiply); };
  ...
  void Concatenate(mafTrasformBase *trans, int premultiply) {Concatenate(trans->GetMatrix(),premultiply);}
  ...
  static void GetVersor(int axis, const vtkMatrix4x4 *matrix, float versor[3]) \
    { \ 
      double temp[3]; mafTransform::GetVersor(axis, matrix, temp); \
...
protected:
  ...
  int InverseFlag;
  mafTrasformBase  *Input;

  mafTrasformBase  *InputFrame;
  mafTrasformBase  *TargetFrame;

  double	Sign(double a); 

private:
  mafTransform(const mafTransform&);  // Not implemented.
  void operator=(const mafTransform&);  // Not implemented.
};
