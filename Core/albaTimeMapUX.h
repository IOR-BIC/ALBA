...
template <class T>
class ALBA_EXPORT albaTimeMap : public albaObject, public albaTimeStamped
{
public:
...
//-------------------------------------------------------------------------
{
  ...
  for (typename albaTimeMap<T>::TimeMap::const_iterator it=m_TimeMap.begin();it!=m_TimeMap.end();it++)
  {
...
//-------------------------------------------------------------------------
{
  ...
  typename albaTimeMap<T>::TimeMap::iterator it;
...
//-------------------------------------------------------------------------
{
  ...
  typename albaTimeMap<T>::TimeMap::iterator it;
  typename albaTimeMap<T>::TimeMap::iterator it2;
...
//-------------------------------------------------------------------------
{
  typename albaTimeMap<T>::TimeMap::iterator it=FindItemByIndex(idx);
  if (it!=m_TimeMap.end())
  {
    RemoveItem(it);
    return ALBA_OK;
...
//-------------------------------------------------------------------------
{
  std::pair<typename albaTimeMap<T>::TimeMap::iterator,typename albaTimeMap<T>::TimeMap::iterator> range=m_TimeMap.equal_range(t);
...
//-------------------------------------------------------------------------
{
  if (m_TimeMap.size()>0)
  {
    typename albaTimeMap<T>::TimeMap::iterator it=m_TimeMap.lower_bound(t); // find first item >= t
...
//----------------------------------------------------------------------------
{
  ...
  {
     typename albaTimeMap<T>::TimeMap::iterator it=m_TimeMap.begin();
     std::advance(it,idx);
     return it;
  }
...
//----------------------------------------------------------------------------
{
  typename albaTimeMap<T>::TimeMap::iterator it=FindItem(t);
...
//----------------------------------------------------------------------------
{
  typename albaTimeMap<T>::TimeMap::iterator it=FindNearestItem(t);
  return (it!=m_TimeMap.end())?it->second:NULL;
}
//----------------------------------------------------------------------------
template <class T>
...
//----------------------------------------------------------------------------
{
  typename albaTimeMap<T>::TimeMap::iterator it=FindItemBefore(t);
  return (it!=m_TimeMap.end())?it->second:NULL;
}
