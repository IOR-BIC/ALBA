/*=========================================================================
  ...
  Module:    $RCSfile: medOpImporterCTA.cpp,v $
  Language:  C++
...
int compareX(const medOpImporterCTAListElement **arg1,const medOpImporterCTAListElement **arg2);
int compareY(const medOpImporterCTAListElement **arg1,const medOpImporterCTAListElement **arg2);
int compareZ(const medOpImporterCTAListElement **arg1,const medOpImporterCTAListElement **arg2);
int compareTriggerTime(const medOpImporterCTAListElement **arg1,const medOpImporterCTAListElement **arg2);
int compareImageNumber(const medOpImporterCTAListElement **arg1,const medOpImporterCTAListElement **arg2);

WX_DEFINE_LIST(ListCTAFiles);

//----------------------------------------------------------------------------
mafCxxTypeMacro(medOpImporterCTA);
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
// Constants :
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
medOpImporterCTA::medOpImporterCTA(wxString label) : mafOp(label)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
medOpImporterCTA::~medOpImporterCTA()
//----------------------------------------------------------------------------
{
	vtkDEL(m_Volume);
  vtkDEL(m_Image);
...
//----------------------------------------------------------------------------
mafOp *medOpImporterCTA::Copy()
//----------------------------------------------------------------------------
{
	return new medOpImporterCTA(m_Label);
}
//----------------------------------------------------------------------------
void medOpImporterCTA::OpRun()   
//----------------------------------------------------------------------------
{
 	CreatePipeline();
	CreateGui();
...
//----------------------------------------------------------------------------
void medOpImporterCTA::WaitUser()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::OpStop(int result)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::CreatePipeline()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::CreateGui()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::BuildDicomFileList(const char *dir)
//----------------------------------------------------------------------------
{
	int row, i;
	double slice_pos[3];
		...
		for (i=0; i < m_CTDirectoryReader->GetNumberOfFiles(); i++)
		{
			...
			else
			{
				...
				if (row == -1)
				{
					...
					m_FilesList->Append(new medOpImporterCTAListElement(str_tmp,slice_pos));
					m_NumberOfStudy++;
				}
				else 
				{
					...
					((ListCTAFiles *)m_StudyListbox->GetClientData(row))->Append(new medOpImporterCTAListElement(str_tmp,slice_pos));
				}
				}
			...
			else
			{
				...
				//if (strcmp(reader->GetCTMode(),"SCOUT MODE") == 0 || reader->GetStatus() == -1)
					...
					if (row == -1)
					{
						...
						//medOpImporterDICOMListElement *element = new medOpImporterDICOMListElement(str_tmp,slice_pos);
						//m_FilesList->Append(element);
						m_FilesList->Append(new medOpImporterCTAListElement(str_tmp,slice_pos,imageNumber, cardNumImages, trigTime));
						m_NumberOfStudy++;
					}
					else 
					{
						...
						//medOpImporterDICOMListElement *element = new medOpImporterDICOMListElement(str_tmp,SlicePos);
						//((ListCTAFiles *)m_StudyListbox->GetClientData(row))->Append(element);
						((ListCTAFiles *)m_StudyListbox->GetClientData(row))->Append(new medOpImporterCTAListElement(str_tmp,slice_pos,imageNumber,cardNumImages,trigTime));
					}
				}
				progress = i * 100 / m_CTDirectoryReader->GetNumberOfFiles();
				mafEventMacro(mafEvent(this,PROGRESSBAR_SET_VALUE,progress));
...
//----------------------------------------------------------------------------
void medOpImporterCTA::BuildVolumeCineMRI()
//----------------------------------------------------------------------------
{
	int step;
  ...
  for (int ts = 0; ts < m_NumberOfTimeFrames; ts++)
  {
    ...
    medOpImporterCTAListElement *element0;
    element0 = (medOpImporterCTAListElement *)m_ListSelected->Item(tsImageId)->GetData();
...
//----------------------------------------------------------------------------
void medOpImporterCTA::BuildVolume()
//----------------------------------------------------------------------------
{
	int step;
...
//----------------------------------------------------------------------------
void medOpImporterCTA::ImportDicomTags()
//----------------------------------------------------------------------------
{
  if (m_TagArray == NULL) 
    mafNEW(m_TagArray);
...
//----------------------------------------------------------------------------
void medOpImporterCTA::ShowSlice(int slice_num)
//----------------------------------------------------------------------------
{
	// Description:
...
//----------------------------------------------------------------------------
void medOpImporterCTA::ResetSliders()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::ResetStructure()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double x1 = (*(medOpImporterCTAListElement **)arg1)->GetCoordinate(0);
	double x2 = (*(medOpImporterCTAListElement **)arg2)->GetCoordinate(0);
	if (x1 > x2)
		return 1;
	if (x1 < x2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double y1 = (*(medOpImporterCTAListElement **)arg1)->GetCoordinate(1);
	double y2 = (*(medOpImporterCTAListElement **)arg2)->GetCoordinate(1);
	if (y1 > y2)
		return 1;
	if (y1 < y2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double z1 = (*(medOpImporterCTAListElement **)arg1)->GetCoordinate(2);
	double z2 = (*(medOpImporterCTAListElement **)arg2)->GetCoordinate(2);
	if (z1 > z2)
		return 1;
	if (z1 < z2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	float t1 = (*(medOpImporterCTAListElement **)arg1)->GetTriggerTime();
	float t2 = (*(medOpImporterCTAListElement **)arg2)->GetTriggerTime();;
	if (t1 > t2)
		return 1;
	if (t1 < t2)
		return -1;
...
//----------------------------------------------------------------------------
{
	// compare the trigger time of both arguments
	// return:
	float i1 = (*(medOpImporterCTAListElement **)arg1)->GetImageNumber();
	float i2 = (*(medOpImporterCTAListElement **)arg2)->GetImageNumber();;
	if (i1 > i2)
		return 1;
	if (i1 < i2)
		return -1;
...
//----------------------------------------------------------------------------
void medOpImporterCTA::	OnEvent(mafEventBase *maf_event) 
//----------------------------------------------------------------------------
{
    ...
    switch(e->GetId())
    {
      ...
      case ID_STUDY:
      {
        ...
        if(m_ListSelected->GetCount() > 1)
        {
          ...
          medOpImporterCTAListElement *element1;
          medOpImporterCTAListElement *element2;
          element1 = (medOpImporterCTAListElement *)m_ListSelected->Item(0)->GetData();
          element2 = (medOpImporterCTAListElement *)m_ListSelected->Item(1)->GetData();
        ...
        case 2:
          ...
          break;
				...
				m_NumberOfTimeFrames = ((medOpImporterCTAListElement *)m_ListSelected->Item(0)->GetData())->GetCardiacNumberOfImages();
        if(m_DICOMType == ID_CMRI) //If cMRI
...
//----------------------------------------------------------------------------
int medOpImporterCTA::GetImageId(int timeId, int heigthId)
//----------------------------------------------------------------------------
{
  /* 
  ...
  */
  ...
  medOpImporterCTAListElement *element0;
  element0 = (medOpImporterCTAListElement *)m_ListSelected->Item(0)->GetData();
...
//----------------------------------------------------------------------------
void medOpImporterCTA::SetParameters(void *param)
//----------------------------------------------------------------------------
{
	if(param)
	{
...
//----------------------------------------------------------------------------
void medOpImporterCTA::SetDirectory(mafString dir)
//----------------------------------------------------------------------------
{
	m_DICOMDir=dir;
}
