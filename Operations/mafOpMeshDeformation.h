...
//----------------------------------------------------------------------------
/** */
class MAF_EXPORT mafOpMeshDeformation: public mafOp
{
protected:
  //----------------------------------------------------------------------------
  // Constants:
...
#pragma region //Help structures
  typedef struct UNDO_ITEM 
  {
    ...
    mafPolylineGraph* pOC_DC[2];  //<original and deformed curve (in unmodified version)    
    vtkIdList* pCC;               //<correspondences (in unmodified version)
  } UNDO_ITEM;
  
  typedef struct CONTROL_CURVE
  {  
    mafPolylineGraph* pPolyLines[2];    //<polyline curves (0 = original, 1 = deformed)
...
protected:
  ...
  It constructs also the VTK pipeline.*/
  CONTROL_CURVE* CreateControlCurve(mafPolylineGraph* pOC, 
...
#pragma region //Edit operations
  ...
  void AddControlCurve(mafPolylineGraph* pOC, mafPolylineGraph* pDC, vtkIdList* pCC);
...
protected:
  ...
  mafPolylineGraph* CreateCopyOfPolylineGraph(mafPolylineGraph* input);
...
#pragma region //Visualization stuff
  ...
  mafVME* m_OCToAdd;    //<this is the VME of the original curve selected to be added
  mafVME* m_DCToAdd;    //<this is the VME of the deformed curve selected to be added
