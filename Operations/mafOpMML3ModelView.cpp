...
#include "mafOpMML3ModelView2DPipe.h"
#include "mafOpMML3ModelView3DPipe.h"
...
//------------------------------------------------------------------------------
mafOpMML3ModelView::mafOpMML3ModelView( vtkRenderWindow *rw, vtkRenderer *ren, vtkPolyData *muscleIn, vtkPolyData *muscleOut,
...
{ 
  ...
  // Create visual pipes
  //----------------------------------------------------------------------------
  m_VisualPipe2D = new mafOpMML3ModelView2DPipe(ren, muscleIn, volume, m_NumberOfScans) ;
  m_VisualPipe3D = new mafOpMML3ModelView3DPipe(ren, muscleIn, volume, m_NumberOfScans) ;
  m_VisualPipePreview = new mafOpMML3ModelView3DPipe(ren, muscleOut, volume, m_NumberOfScans) ;
...
//----------------------------------------------------------------------------
// Destructor
mafOpMML3ModelView::~mafOpMML3ModelView()
//----------------------------------------------------------------------------
{
  m_Math->Delete() ;
...
void mafOpMML3ModelView::Initialize()
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::SetDisplay2D()
//------------------------------------------------------------------------------
{
  m_Display3D = 0 ;
...
void mafOpMML3ModelView::SetDisplay3D()
//------------------------------------------------------------------------------
{
  m_Display3D = 1 ;
...
void mafOpMML3ModelView::SetDisplayToPreview()
//------------------------------------------------------------------------------
{
  m_Display3D = 2 ;
...
void mafOpMML3ModelView::ResetCameraPosition()
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::SetCurrentIdOfScans(int i) 
//------------------------------------------------------------------------------
{
  m_ScansCurrentId = i ;
...
void mafOpMML3ModelView::FindSizeAndResolutionOfScans()
//------------------------------------------------------------------------------
{
...
double mafOpMML3ModelView::LengthOfAxis() const
//----------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::SetFractionalPosOfScans(double *alpha)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::CalculatePositionsOfScans()
//------------------------------------------------------------------------------
{
  double pstart[3] ; // low insertion
  double pmid[3] ;   // mid landmark if two-part axis
...
void mafOpMML3ModelView::CalculateNormalsOfScans()
//------------------------------------------------------------------------------
{
  double pstart[3] ; // low insertion
  double pmid[3] ;   // mid landmark if two-part axis
...
void mafOpMML3ModelView::CalculatePoseMatricesOfScans()
//----------------------------------------------------------------------------
{
  for (int i = 0 ;  i < m_NumberOfScans ;  i++){
...
void mafOpMML3ModelView::CalculateOriginalContourCenters()
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::GetOriginalContourBounds(double bounds[6])
//------------------------------------------------------------------------------
{
  for (int i = 0 ;  i < 6 ;  i++)
...
// Returns the value saved by CalculateOriginalContourCenters()
void mafOpMML3ModelView::GetOriginalContourCenter(double center[3])
//------------------------------------------------------------------------------
{
  for (int i = 0 ;  i < 3 ;  i++)
...
double mafOpMML3ModelView::GetZetaOfSlice(int sliceId) const
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
// Get distance zeta along slice axis
double mafOpMML3ModelView::GetZetaOfCurrentSlice() const
//----------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::CalculateSlicePosition(double zeta, double *pos) const
//------------------------------------------------------------------------------
{
  double pstart[3] ; // low insertion
  double pmid[3] ;   // mid landmark if two-part axis
...
void mafOpMML3ModelView::CalculateSliceNormal(double zeta, double *normal) const
//------------------------------------------------------------------------------
{
  double alpha = zeta / LengthOfAxis() ;
...
void mafOpMML3ModelView::CalculateSlicePoseMatrix(double zeta, vtkMatrix4x4 *mat) const
//------------------------------------------------------------------------------
{
...
double mafOpMML3ModelView::HeightAbovePlane(double *normal, double *origin, double *x) const
//------------------------------------------------------------------------------
{
...
// the value is zero for a point in the plane.
double mafOpMML3ModelView::HeightAbovePlane(double zeta, double *x) const
//------------------------------------------------------------------------------
{
...
double mafOpMML3ModelView::CalculateZetaOfPoint(double *x) const
//----------------------------------------------------------------------------
{
  double zeta ;
...
void mafOpMML3ModelView::ApplyRegistrationOps()
//----------------------------------------------------------------------------
{
  // number of points
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::Render()
//----------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::SetText(int m, double n, int d, int s)
//----------------------------------------------------------------------------
{
  char text[10];
...
//----------------------------------------------------------------------------
int mafOpMML3ModelView::GetTotalNumberOfScans() const
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetResolutionOfScans(int *x, int *y) const
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetSizeOfScans(double *x, double *y) const
//----------------------------------------------------------------------------
{
  *x = m_ScanSizeX;
  *y = m_ScanSizeY;
...
//------------------------------------------------------------------------------
// Update everything
void mafOpMML3ModelView::Update()
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::UpdateCuttingPlanesTransform()
//------------------------------------------------------------------------------
{
  vtkTransform* Transform = vtkTransform::New();
  Transform->Identity();
...
void mafOpMML3ModelView::UpdateContourAxesTransform()
//------------------------------------------------------------------------------
{
  // get zeta level
  double zeta = GetZetaOfCurrentSlice();
...
void mafOpMML3ModelView::UpdateGlobalAxesTransform()
//----------------------------------------------------------------------------
{
  // set transform to identity
  vtkTransform* Transform = vtkTransform::New();
...
void mafOpMML3ModelView::UpdateSegmentNorthEastTransform()
//----------------------------------------------------------------------------
{
  // get zeta level
  double zeta = GetZetaOfCurrentSlice();
...
void mafOpMML3ModelView::UpdateSegmentNorthWestTransform()
//----------------------------------------------------------------------------
{
  // get zeta level
  double zeta = GetZetaOfCurrentSlice();
...
void mafOpMML3ModelView::UpdateSegmentSouthEastTransform()
//----------------------------------------------------------------------------
{
  // get zeta level
  double zeta = GetZetaOfCurrentSlice();
...
void mafOpMML3ModelView::UpdateSegmentSouthWestTransform()
//----------------------------------------------------------------------------
{
  // get zeta level
  double zeta = GetZetaOfCurrentSlice();
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark1OfAtlas(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark2OfAtlas(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark3OfAtlas(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark4OfAtlas(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark1OfAtlas(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark2OfAtlas(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark3OfAtlas(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark4OfAtlas(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark1OfPatient(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark2OfPatient(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark3OfPatient(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark4OfPatient(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark1OfPatient(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark2OfPatient(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark3OfPatient(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark4OfPatient(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark1OfAxis(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark2OfAxis(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::SetLandmark3OfAxis(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark1OfAxis(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark2OfAxis(double *xyz) const 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::GetLandmark3OfAxis(double *xyz) const 
//----------------------------------------------------------------------------
{
...
void mafOpMML3ModelView::MultiplyMatrixPoint(vtkMatrix4x4* A, double b[3], double c[3]) const
//------------------------------------------------------------------------------
{
  int i, j ;
  double ctemp[4] ;
...
void mafOpMML3ModelView::CalculateCenterOfVtkPlane(double sizx, double sizy, double p[3]) const
//------------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mafOpMML3ModelView::Print(vtkObject *obj, wxString msg) const
//----------------------------------------------------------------------------
{
  wxLogMessage("%s",msg);
...
void mafOpMML3ModelView::AllocateOperationsStack(int numberOfComponents, int numberOfTuples)
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Print self
void mafOpMML3ModelView::PrintSelf(ostream &os, int indent)
//------------------------------------------------------------------------------
{
