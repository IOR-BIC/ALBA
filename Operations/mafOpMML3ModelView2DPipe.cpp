/*=========================================================================
 ...
 Program: MAF2
 Module: mafOpMML3ModelView2DPipe
...
// Constructor
//------------------------------------------------------------------------------
mafOpMML3ModelView2DPipe::mafOpMML3ModelView2DPipe(vtkRenderer *ren, vtkPolyData *muscle, vtkDataSet* volume, int numberOfSlices)
...
//------------------------------------------------------------------------------
// Destructor
mafOpMML3ModelView2DPipe::~mafOpMML3ModelView2DPipe()
//------------------------------------------------------------------------------
{
  // look up table
  m_Lut->Delete() ;
...
//------------------------------------------------------------------------------
// Set visibility
void mafOpMML3ModelView2DPipe::SetVisibility(int visibility)
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Update
void mafOpMML3ModelView2DPipe::Update()
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::UpdateAllSlices()
//------------------------------------------------------------------------------
{
  for (int i = 0 ;  i < m_NumberOfSlices ;  i++){
...
void mafOpMML3ModelView2DPipe::SetCurrentSliceId(int i)
//------------------------------------------------------------------------------
{
  m_CurrentSliceId = i ;
  SetVisibility(m_CurrentVisibility) ;
...
void mafOpMML3ModelView2DPipe::SetSliceTransform(int i, vtkMatrix4x4 *mat)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::SetTubeRadius(double r)
//------------------------------------------------------------------------------
{
  m_TubeSize = r ;
...
//------------------------------------------------------------------------------
// set x text
void mafOpMML3ModelView2DPipe::SetTextXVisibility(int visibility)
//------------------------------------------------------------------------------
{
  m_TextXOn = true ;
...
//------------------------------------------------------------------------------
// set y text
void mafOpMML3ModelView2DPipe::SetTextYVisibility(int visibility)
//------------------------------------------------------------------------------
{
  m_TextYOn = true ;
...
//------------------------------------------------------------------------------
// set lut range
void mafOpMML3ModelView2DPipe::SetLutRange(double scalarMin, double scalarMax)
//------------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
// set up contour axes
void mafOpMML3ModelView2DPipe::SetUpContourCoordinateAxes()
//----------------------------------------------------------------------------
{
  // east
...
//----------------------------------------------------------------------------
// set up global axes
void mafOpMML3ModelView2DPipe::SetUpGlobalCoordinateAxes()
//----------------------------------------------------------------------------
{
  // east
...
void mafOpMML3ModelView2DPipe::SetSliceResolution(int resX, int resY)
//------------------------------------------------------------------------------
{
  m_ResX = resX ;
  m_ResY = resY ;
...
void mafOpMML3ModelView2DPipe::SetSliceSize(double sizeX, double sizeY)
//------------------------------------------------------------------------------
{
  m_SizeX = sizeX ;
  m_SizeY = sizeY ;
...
void mafOpMML3ModelView2DPipe::SetContourAxesVisibility(int visibility)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::SetGlobalAxesVisibility(int visibility)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::TransformPointToPatient(double *p)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::SetContourAxesTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::SetGlobalAxesTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::GetOriginalSegmentBoundsNE(double *bounds) 
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Get the bounds of the contour quadrant
void mafOpMML3ModelView2DPipe::GetOriginalSegmentBoundsNW(double *bounds) 
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Get the bounds of the contour quadrant
void mafOpMML3ModelView2DPipe::GetOriginalSegmentBoundsSE(double *bounds) 
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Get the bounds of the contour quadrant
void mafOpMML3ModelView2DPipe::GetOriginalSegmentBoundsSW(double *bounds) 
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::GetOriginalSegmentCenterNE(double *center) 
//------------------------------------------------------------------------------
{
  double bounds[6] ;
...
//------------------------------------------------------------------------------
// Get center of contour quadrant after clipping, before transform
void mafOpMML3ModelView2DPipe::GetOriginalSegmentCenterNW(double *center) 
//------------------------------------------------------------------------------
{
  double bounds[6] ;
...
//------------------------------------------------------------------------------
// Get center of contour quadrant after clipping, before transform
void mafOpMML3ModelView2DPipe::GetOriginalSegmentCenterSE(double *center) 
//------------------------------------------------------------------------------
{
  double bounds[6] ;
...
//------------------------------------------------------------------------------
// Get center of contour quadrant after clipping, before transform
void mafOpMML3ModelView2DPipe::GetOriginalSegmentCenterSW(double *center) 
//------------------------------------------------------------------------------
{
  double bounds[6] ;
...
void mafOpMML3ModelView2DPipe::GetCurrentContourBounds(double *bounds) 
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::GetCurrentContourCenter(double *center) 
//------------------------------------------------------------------------------
{
  double bounds[6] ;
...
void mafOpMML3ModelView2DPipe::SetCuttingPlanesTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
// This is the method which applies the effect of the widget to the contour
void mafOpMML3ModelView2DPipe::SetNEContourTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
// Set the transform of the contour quadrant
// This is the method which applies the effect of the widget to the contour
void mafOpMML3ModelView2DPipe::SetSEContourTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
// Set the transform of the contour quadrant
// This is the method which applies the effect of the widget to the contour
void mafOpMML3ModelView2DPipe::SetNWContourTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
// Set the transform of the contour quadrant
// This is the method which applies the effect of the widget to the contour
void mafOpMML3ModelView2DPipe::SetSWContourTransform(vtkTransform *transform)
//------------------------------------------------------------------------------
{
...
void mafOpMML3ModelView2DPipe::CalculateRobustCenterOfContour(double *pos) const
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
// Print self
void mafOpMML3ModelView2DPipe::PrintSelf(ostream& os, int indent)
//------------------------------------------------------------------------------
{
  Update() ;
