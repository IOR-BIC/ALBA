...
#ifndef __mafOpRegisterClusters_H__
#define __mafOpRegisterClusters_H__
...
The source is deep copied into new VMEs (in a new mafVMEGroup) and the resulting transformation is applied 
...
class MAF_EXPORT mafOpRegisterClusters: public mafOp
{
public:
	//----------------------------------------------------------------------------
	// Constants :
...
#pragma endregion
public:
  mafOpRegisterClusters(wxString label = _("Register Landmark Cloud"));
 ~mafOpRegisterClusters(); 

 mafTypeMacro(mafOpRegisterClusters, mafOp);
  ...
  void OpUndo();	
	...
	inline void SetSource(mafVMELandmarkCloud *source){
		this->SetInput(source);
	}
	
	/** Helper method. Gets the source landmark cloud. */
	inline mafVMELandmarkCloud* GetSource() const {
		return mafVMELandmarkCloud::SafeDownCast(m_Input);
	}

	/** Sets the target landmark cloud. */
  virtual void SetTarget(mafVMELandmarkCloud *target);

	/** Gets the target landmark cloud. */
	inline virtual mafVMELandmarkCloud* GetTarget() const {
		return m_Target;
	}
  ...
  virtual void SetFollower(mafVMESurface *follower);

	/** Gets the target landmark cloud. */
	inline virtual mafVMESurface* GetFollower() const {
		return m_Follower;
	}
  ...
  inline virtual mafVMEGroup* GetResult(){
		return m_Result;
	}
...
protected:	
	...
	mafVMELandmarkCloud*    m_Target;			///<target landmark cloud
	mafVMELandmarkCloud*    m_Registered;	///<the result of the registration of source to target
	
	mafVMESurface*					m_Follower;						///<surface follower of source
	mafVMESurface*					m_RegisteredFollower;	///<the result of the registration of source to target

  mafVMEGroup*						m_Result;			///<group where the results are stored
  mafVMEInfoText*					m_Info;				///<report of registration
...
#pragma region GUI
	mafGUIDialog *m_Dialog;			///<main dialog
	mafString   m_SourceName;		///<name of source
	mafString   m_TargetName;		///<name of target
	mafString   m_FollowerName;	///<name of follower
