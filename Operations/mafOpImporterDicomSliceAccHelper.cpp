...
void mafOpImporterDicomSliceAccHelper::SetSlice(int slice_num,vtkImageData * slice,double *unRotatedOrigin)
//--------------------------------------------------------------------------------------
{
	int dimensions[3];
	...
	if (slice_num >=0 && slice_num < m_NumOfSlices)
	{
		...
		{
			mafLogMessage("Bad dimensions of input slice");
			return;
		}
...
//----------------------------------------------------------------------------
vtkDataSet* mafOpImporterDicomSliceAccHelper::GetNewOutput()
{
	bool useRG = false;
...
//----------------------------------------------------------------------------
void mafOpImporterDicomSliceAccHelper::SetDims(int *dims)
{
	m_Dims[0] = dims[0];
	m_Dims[1] = dims[1];
	m_Dims[2] = dims[2];
...
//----------------------------------------------------------------------------
void mafOpImporterDicomSliceAccHelper::SetOrigin(double *origin)
{
	m_Origin[0] = origin[0];
	m_Origin[1] = origin[1];
	m_Origin[2] = origin[2];
...
//----------------------------------------------------------------------------
void mafOpImporterDicomSliceAccHelper::SetSpacing(double *spacing)
{
...
//--------------------------------------------------------------------------------------
void mafOpImporterDicomSliceAccHelper::Allocate()
{
	if (m_NumOfSlices == 0)
		return;
