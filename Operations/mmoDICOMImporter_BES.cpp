...
int compareX(const mmoDICOMImporter_BESListElement **arg1,const mmoDICOMImporter_BESListElement **arg2);
int compareY(const mmoDICOMImporter_BESListElement **arg1,const mmoDICOMImporter_BESListElement **arg2);
int compareZ(const mmoDICOMImporter_BESListElement **arg1,const mmoDICOMImporter_BESListElement **arg2);
int compareTriggerTime(const mmoDICOMImporter_BESListElement **arg1,const mmoDICOMImporter_BESListElement **arg2);
int compareImageNumber(const mmoDICOMImporter_BESListElement **arg1,const mmoDICOMImporter_BESListElement **arg2);

WX_DEFINE_LIST(ListDicomFiles);

//----------------------------------------------------------------------------
mafCxxTypeMacro(mmoDICOMImporter_BES);
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
mmoDICOMImporter_BES::mmoDICOMImporter_BES(wxString label) : mafOp(label)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
mmoDICOMImporter_BES::~mmoDICOMImporter_BES()
//----------------------------------------------------------------------------
{
#ifdef VME_VOLUME_LARGE
...
//----------------------------------------------------------------------------
mafOp *mmoDICOMImporter_BES::Copy()
//----------------------------------------------------------------------------
{
	return new mmoDICOMImporter_BES(m_Label);
}
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::OpRun()   
//----------------------------------------------------------------------------
{
	CreatePipeline();
	CreateGui();
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::OpStop(int result)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::OpDo()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::OpUndo()
//----------------------------------------------------------------------------
{   
	//assert(m_Volume != NULL);
  //assert(m_Image != NULL);
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::CreatePipeline()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::CreateGui()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::BuildDicomFileList(const char *dir)
//----------------------------------------------------------------------------
{
	int row, i;
	double slice_pos[3];
			...
			else
			{
				...
				if (strcmp( reader->GetModality(), "CT" ) == 0 || strcmp( reader->GetModality(), "XA" )==0)
				{
					...
					if (row == -1)
					{
						...
						//mmoDICOMImporter_BESListElement *element = new mmoDICOMImporter_BESListElement(str_tmp,slice_pos);
						//m_FilesList->Append(element);
						m_FilesList->Append(new mmoDICOMImporter_BESListElement(str_tmp,slice_pos));
						m_NumberOfStudy++;
					}
					else 
					{
						reader->GetSliceLocation(slice_pos);
						//mmoDICOMImporter_BESListElement *element = new mmoDICOMImporter_BESListElement(str_tmp,SlicePos);
						//((ListDicomFiles *)m_StudyListbox->GetClientData(row))->Append(element);
						((ListDicomFiles *)m_StudyListbox->GetClientData(row))->Append(new mmoDICOMImporter_BESListElement(str_tmp,slice_pos));
					}
				}
			...
			else
			{
				...
				//if (strcmp(reader->GetCTMode(),"SCOUT MODE") == 0 || reader->GetStatus() == -1)
					...
					if (row == -1)
					{
						...
						//mmoDICOMImporter_BESListElement *element = new mmoDICOMImporter_BESListElement(str_tmp,slice_pos);
						//m_FilesList->Append(element);
						m_FilesList->Append(new mmoDICOMImporter_BESListElement(str_tmp,slice_pos,imageNumber, cardNumImages, trigTime));
						m_NumberOfStudy++;
					}
					else 
					{
						...
						//mmoDICOMImporter_BESListElement *element = new mmoDICOMImporter_BESListElement(str_tmp,SlicePos);
						//((ListDicomFiles *)m_StudyListbox->GetClientData(row))->Append(element);
						((ListDicomFiles *)m_StudyListbox->GetClientData(row))->Append(new mmoDICOMImporter_BESListElement(str_tmp,slice_pos,imageNumber,cardNumImages,trigTime));
					}
				}
				progress = i * 100 / m_CTDirectoryReader->GetNumberOfFiles();
				mafEventMacro(mafEvent(this,PROGRESSBAR_SET_VALUE,progress));
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::BuildVolumeCineMRI()
//----------------------------------------------------------------------------
{
	int step;
  ...
  for (int ts = 0; ts < m_NumberOfTimeFrames; ts++)
  {
    ...
    mmoDICOMImporter_BESListElement *element0;
    element0 = (mmoDICOMImporter_BESListElement *)m_ListSelected->Item(tsImageId)->GetData();
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::BuildVolume()
//----------------------------------------------------------------------------
{
  int step;
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::ImportDicomTags()
//----------------------------------------------------------------------------
{
  if (m_TagArray == NULL) 
    mafNEW(m_TagArray);
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::ShowSlice(int slice_num)
//----------------------------------------------------------------------------
{
	// Description:
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::ResetSliders()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::ResetStructure()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double x1 = (*(mmoDICOMImporter_BESListElement **)arg1)->GetCoordinate(0);
	double x2 = (*(mmoDICOMImporter_BESListElement **)arg2)->GetCoordinate(0);
	if (x1 > x2)
		return 1;
	if (x1 < x2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double y1 = (*(mmoDICOMImporter_BESListElement **)arg1)->GetCoordinate(1);
	double y2 = (*(mmoDICOMImporter_BESListElement **)arg2)->GetCoordinate(1);
	if (y1 > y2)
		return 1;
	if (y1 < y2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	double z1 = (*(mmoDICOMImporter_BESListElement **)arg1)->GetCoordinate(2);
	double z2 = (*(mmoDICOMImporter_BESListElement **)arg2)->GetCoordinate(2);
	if (z1 > z2)
		return 1;
	if (z1 < z2)
		return -1;
...
//----------------------------------------------------------------------------
{
	...
	// return:
	float t1 = (*(mmoDICOMImporter_BESListElement **)arg1)->GetTriggerTime();
	float t2 = (*(mmoDICOMImporter_BESListElement **)arg2)->GetTriggerTime();;
	if (t1 > t2)
		return 1;
	if (t1 < t2)
		return -1;
...
//----------------------------------------------------------------------------
{
	// compare the trigger time of both arguments
	// return:
	float i1 = (*(mmoDICOMImporter_BESListElement **)arg1)->GetImageNumber();
	float i2 = (*(mmoDICOMImporter_BESListElement **)arg2)->GetImageNumber();;
	if (i1 > i2)
		return 1;
	if (i1 < i2)
		return -1;
...
//----------------------------------------------------------------------------
void mmoDICOMImporter_BES::	OnEvent(mafEventBase *maf_event) 
//----------------------------------------------------------------------------
{
    ...
    switch(e->GetId())
    {
      ...
      case ID_STUDY:
      {
        ...
        if(m_ListSelected->GetCount() > 1)
        {
          ...
          mmoDICOMImporter_BESListElement *element1;
          mmoDICOMImporter_BESListElement *element2;
          element1 = (mmoDICOMImporter_BESListElement *)m_ListSelected->Item(0)->GetData();
          element2 = (mmoDICOMImporter_BESListElement *)m_ListSelected->Item(1)->GetData();
        ...
        case 2:
          ...
          break;
				...
				m_NumberOfTimeFrames = ((mmoDICOMImporter_BESListElement *)m_ListSelected->Item(0)->GetData())->GetCardiacNumberOfImages();
        if(m_DICOMType == ID_CMRI) //If cMRI
...
//----------------------------------------------------------------------------
int mmoDICOMImporter_BES::GetImageId(int timeId, int heigthId)
//----------------------------------------------------------------------------
{
  /* 
  ...
  */
  ...
  mmoDICOMImporter_BESListElement *element0;
  element0 = (mmoDICOMImporter_BESListElement *)m_ListSelected->Item(0)->GetData();
...
vtkImageData* mmoDICOMImporter_BES::GetImageData(vtkRectilinearGrid* pInput)
//------------------------------------------------------------------------
{
  if (m_BuildRLG != 0)
...
bool mmoDICOMImporter_BES::DetectSpacing(vtkDoubleArray* pCoords, double* pOutSpacing)
//------------------------------------------------------------------------
{
