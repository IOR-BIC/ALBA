...
#pragma endregion //wxFormBuilder
  ...
  m_BttnSelDC->SetValidator(mafGUIValidator(this, ID_SELECT_DC, m_BttnSelDC));
  m_BttnResetCurve->SetValidator(mafGUIValidator(this, ID_RESETCURVE, m_BttnResetCurve));
  m_BttnAddCurve->SetValidator(mafGUIValidator(this, ID_ADDCURVE, m_BttnAddCurve));
  m_BttnReset->SetValidator(mafGUIValidator(this, ID_RESET, m_BttnReset));
  m_BttnUndo->SetValidator(mafGUIValidator(this, ID_UNDO, m_BttnUndo));
  m_BttnPreview->SetValidator(mafGUIValidator(this, ID_PREVIEW, m_BttnPreview));

  //validators for EditMode & Deformation Mode
  editMode->SetValidator(mafGUIValidator(this, ID_EDITMODE, editMode, &m_EditMode));
  OnEditMode(); //to force Help info

  defMode->SetValidator(mafGUIValidator(this, ID_DEFORMATIONMODE, defMode, &m_DeformationMode));
  OnDeformationMode(); //to force Help info

  //validators for visual options
  chckShowOM->SetValidator(mafGUIValidator(this, ID_SHOW_XX_CHANGE, chckShowOM, &m_MeshesVisibility[0]));
  chckShowDM->SetValidator(mafGUIValidator(this, ID_SHOW_XX_CHANGE, chckShowDM, &m_MeshesVisibility[1]));
  chckShowOC->SetValidator(mafGUIValidator(this, ID_SHOW_XX_CHANGE, chckShowOC, &m_CurvesVisibility[0]));
  chckShowDC->SetValidator(mafGUIValidator(this, ID_SHOW_XX_CHANGE, chckShowDC, &m_CurvesVisibility[1]));
  chckShowCC->SetValidator(mafGUIValidator(this, ID_SHOW_XX_CHANGE, chckShowCC, &m_CurvesVisibility[2]));

  chckSaveOC->SetValidator(mafGUIValidator(this, ID_SAVEOC, chckSaveOC, &m_SaveODC[0]));
  chckSaveDC->SetValidator(mafGUIValidator(this, ID_SAVEDC, chckSaveDC, &m_SaveODC[1]));

  m_BttnOK->SetValidator(mafGUIValidator(this, ID_OK, m_BttnOK));
...
//------------------------------------------------------------------------
{
  ...
  mafPolylineGraph* pOC = new mafPolylineGraph();
  mafPolylineGraph* pDC = new mafPolylineGraph();
...
void mafOpMeshDeformation::CreateUndo(int index, int nAction, int nFlags)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    else
    {
      ...
      pUndo->pOC_DC[i] = new mafPolylineGraph();
...
//------------------------------------------------------------------------
{
  ...
  mafPolylineGraph* pGraph = m_SelectedCurve->pPolyLines[m_SelectedObjType - SELECT_OC_VERT];
...
//------------------------------------------------------------------------
{
  ...
  mafPolylineGraph* pGraph = pCurve->pPolyLines[iType];
  int nCount = pGraph->GetNumberOfEdges();
  for (int i = 0; i < nCount; i++)
  {
    const mafPolylineGraph::Edge* pEdge = pGraph->GetConstEdgePtr(i);
  ...
  if (nMinEdgeID >= 0)
  {
    ...
    const mafPolylineGraph::Edge* pEdge = pGraph->GetConstEdgePtr(nMinEdgeID);
...
//------------------------------------------------------------------------
{
  ...
  if (nPtID >= 0)
  {
    ...
    mafPolylineGraph* pGraph = pCurve->pPolyLines[iType];
    const mafPolylineGraph::Vertex* pVertex = pGraph->GetConstVertexPtr(nPtID);    
    ...
    else
    {
      ...
      for (int i = 0; i < nCount; i++)
      {
        ...
        const mafPolylineGraph::Vertex* pVA = pGraph->GetConstVertexPtr(pPtr[i]);
        pVA->GetCoords(A);     
    ...
    {      
      ...
      //as mafPolyGraph::DeleteVertex() swaps the last vertex and
    ...
    else
    {
      ...
      else
      {
        ...
        for (int j = 0; j < nCount; j += 2)
        {
          ...
          const mafPolylineGraph::Vertex* pV1 = pCurve->pPolyLines[1]->GetConstVertexPtr(ptIdx[j + 1]);
