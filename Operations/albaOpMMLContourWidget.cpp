...
//----------------------------------------------------------------------------
albaOpMMLContourWidget::albaOpMMLContourWidget() 
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetEnabled(int enabling)
//----------------------------------------------------------------------------
{
  if ( ! this->Interactor )
    {
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                   unsigned long event,
                                   void* clientdata, 
                                   void* vtkNotUsed(calldata))
//----------------------------------------------------------------------------
{
  albaOpMMLContourWidget* self = reinterpret_cast<albaOpMMLContourWidget *>( clientdata );
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::PositionHandles(float X, float Y)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int albaOpMMLContourWidget::HighlightHandle(vtkProp *prop)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::HighlightNormal(int highlight)
//----------------------------------------------------------------------------
{
  if ( highlight )
    {
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::HighlightPlane(int highlight)
//----------------------------------------------------------------------------
{
  if ( highlight )
    {
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnLeftButtonDown()
//----------------------------------------------------------------------------
{
  ...
  if ( ren != this->CurrentRenderer )
    {
    this->m_State = albaOpMMLContourWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
	...
	if (GetScalingMode())
	{
			if (this->m_CurrentHandle)
			{
				...
				{
					// begin s-s op
					this->m_Operation = albaOpMMLContourWidget::SouthScale;
				}
				else
				if (this->m_CurrentHandle == this->m_Handle[1])
				{
					// begin s-e op
					this->m_Operation = albaOpMMLContourWidget::EastScale;
				}
				else
				if (this->m_CurrentHandle == this->m_Handle[2])
				{
					// begin s-w op
					this->m_Operation = albaOpMMLContourWidget::WestScale;
				}
				else
				if (this->m_CurrentHandle == this->m_Handle[3])
				{
					// begin s-n op
					this->m_Operation = albaOpMMLContourWidget::NorthScale;
				}
			}
	}
    ...
    if ( path != NULL )
      ...
      if (/* prop == this->ConeActor || prop == this->LineActor ||
        ...
        {
        this->m_State = albaOpMMLContourWidget::Rotating;
        this->HighlightNormal(1);

		// begin r-op
		this->m_Operation = albaOpMMLContourWidget::Rotation;
        }
     /* else
	  if (prop == this->CenterHandle)
        {
        this->State = albaOpMMLContourWidget::Centralising;
        this->HighlightNormal(1);
        }*/
	  else
        {
		...
		{
			// begin t-op
			this->m_Operation = albaOpMMLContourWidget::Translation;
		}
		else
		if (GetCenterMode())
		{
			// begin p-op
			this->m_Operation = albaOpMMLContourWidget::Placement;
		}
       }
      }
    else
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnLeftButtonUp()
//----------------------------------------------------------------------------
{
  if ( this->m_State == albaOpMMLContourWidget::Outside ||
       this->m_State == albaOpMMLContourWidget::Start )
    {
    return;
    }
  ...
  switch (m_Operation)
  {
	...
	case albaOpMMLContourWidget::SouthScale : //
	...
	case albaOpMMLContourWidget::EastScale : //
	...
	case albaOpMMLContourWidget::WestScale : //
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnMiddleButtonDown()
//----------------------------------------------------------------------------
{
  /*
  int X = this->Interactor->GetEventPosition()[0];
  ...
  if ( ren != this->CurrentRenderer )
    {
    this->State = albaOpMMLContourWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
    {
    this->State = albaOpMMLContourWidget::Pushing;
    this->HighlightPlane(1);
    this->HighlightNormal(1);
    this->HighlightHandle(path->GetFirstNode()->GetProp());
    }
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnMiddleButtonUp()
//----------------------------------------------------------------------------
{
  /*
  if ( this->State == albaOpMMLContourWidget::Outside ||
       this->State == albaOpMMLContourWidget::Start )
    {
    return;
    }
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnRightButtonDown()
//----------------------------------------------------------------------------
{
  /*
  int X = this->Interactor->GetEventPosition()[0];
  ...
  if ( path != NULL )
    {
    this->State = albaOpMMLContourWidget::Scaling;
    this->HighlightPlane(1);
    this->HighlightHandle(path->GetFirstNode()->GetProp());
    }
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnRightButtonUp()
//----------------------------------------------------------------------------
{
  /*
  if ( this->State == albaOpMMLContourWidget::Outside ||
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::OnMouseMove()
//----------------------------------------------------------------------------
{
  ...
  // Process the motion
  if ( this->m_State == albaOpMMLContourWidget::Moving )
  {
  ...
  else if ( this->m_State == albaOpMMLContourWidget::Scaling )
  {
   this->Scale(prevPickPoint, pickPoint, X, Y);
  }
  else if ( this->m_State == albaOpMMLContourWidget::Pushing )
  {
   this->Push(prevPickPoint, pickPoint);
  }
  else if ( this->m_State == albaOpMMLContourWidget::Rotating ) // rotation
  {
   // display information on.
   // off in OnLeftButtonUp()
   m_M->GetScaledTextActor1()->VisibilityOn();
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::Scale(double *p1, double *p2, int vtkNotUsed(X), int Y)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::Push(double *p1, double *p2)
//----------------------------------------------------------------------------
{
  //Get the motion vector
  float v[3];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::CreateDefaultProperties()
//----------------------------------------------------------------------------
{
  // Handle properties
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::PlaceWidget(double bds[6])
//----------------------------------------------------------------------------
{
  int i;
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SelectRepresentation()
//----------------------------------------------------------------------------
{
  if ( ! this->CurrentRenderer )
    {
...
void albaOpMMLContourWidget::SetResolution(int r)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int albaOpMMLContourWidget::GetResolution()
//----------------------------------------------------------------------------
{ 
...
void albaOpMMLContourWidget::SetOrigin(double x, double y, double z) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetOrigin(double x[3]) 
//----------------------------------------------------------------------------
{
  this->SetOrigin(x[0], x[1], x[2]);
}
//----------------------------------------------------------------------------
double* albaOpMMLContourWidget::GetOrigin() 
//----------------------------------------------------------------------------
{
  return this->m_PlaneSource->GetOrigin();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetOrigin(double xyz[3]) 
//----------------------------------------------------------------------------
{
...
void albaOpMMLContourWidget::SetPoint1(double x, double y, double z) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetPoint1(double x[3]) 
//----------------------------------------------------------------------------
{
  this->SetPoint1(x[0], x[1], x[2]);
}
//----------------------------------------------------------------------------
double* albaOpMMLContourWidget::GetPoint1() 
//----------------------------------------------------------------------------
{
  return this->m_PlaneSource->GetPoint1();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetPoint1(double xyz[3]) 
//----------------------------------------------------------------------------
{
...
void albaOpMMLContourWidget::SetPoint2(double x, double y, double z) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetPoint2(double x[3]) 
//----------------------------------------------------------------------------
{
  this->SetPoint2(x[0], x[1], x[2]);
}
//----------------------------------------------------------------------------
double* albaOpMMLContourWidget::GetPoint2() 
//----------------------------------------------------------------------------
{
  return this->m_PlaneSource->GetPoint2();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetPoint2(double xyz[3]) 
//----------------------------------------------------------------------------
{
...
void albaOpMMLContourWidget::SetCenter(double x, double y, double z) 
//----------------------------------------------------------------------------
{
...
// Description:
// Set the center of the plane.
void albaOpMMLContourWidget::SetCenter(double c[3]) 
//----------------------------------------------------------------------------
{
...
double* albaOpMMLContourWidget::GetCenter() 
//----------------------------------------------------------------------------
{
  return this->m_PlaneSource->GetCenter();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetCenter(double xyz[3]) 
//----------------------------------------------------------------------------
{
...
void albaOpMMLContourWidget::SetNormal(double x, double y, double z) 
//----------------------------------------------------------------------------
{
...
// Description:
// Set the normal to the plane.
void albaOpMMLContourWidget::SetNormal(double n[3]) 
//----------------------------------------------------------------------------
{
...
double* albaOpMMLContourWidget::GetNormal() 
//----------------------------------------------------------------------------
{
  return this->m_PlaneSource->GetNormal();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetNormal(double xyz[3]) 
//----------------------------------------------------------------------------
{
  this->m_PlaneSource->GetNormal(xyz);
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::GetPolyData(vtkPolyData *pd)
//----------------------------------------------------------------------------
{ 
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::UpdatePlacement(void)
//----------------------------------------------------------------------------
{
  this->m_PlaneSource->Update();
  this->PositionHandles(0.5, 0.5);
...
void albaOpMMLContourWidget::SetMotionVector(float x, float y, float z) 
//----------------------------------------------------------------------------
{
  this->m_Motion[0] = x;
  this->m_Motion[1] = y;
...
// Description:
// Set the motion vector.
void albaOpMMLContourWidget::SetMotionVector(float m[3]) 
//----------------------------------------------------------------------------
{
  this->m_Motion[0] = m[0];
  this->m_Motion[1] = m[1];
...
// Description:
// Get the motion vector
float* albaOpMMLContourWidget::GetMotionVector() 
//----------------------------------------------------------------------------
{
...
// Description:
// Get the motion vector
void albaOpMMLContourWidget::GetMotionVector(float xyz[3]) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle0(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle1(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle1(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle2(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle2(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle3(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandle3(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
BOOL albaOpMMLContourWidget::GetCenterMode()
//----------------------------------------------------------------------------
{
	return m_BCenterMode;
}
//----------------------------------------------------------------------------
BOOL albaOpMMLContourWidget::GetTranslationMode()
//----------------------------------------------------------------------------
{
	return m_BTranslatingMode;
}
//----------------------------------------------------------------------------
BOOL albaOpMMLContourWidget::GetRotationMode()
//----------------------------------------------------------------------------
{
	return m_BRotatingMode;
}
//----------------------------------------------------------------------------
BOOL albaOpMMLContourWidget::GetScalingMode()
//----------------------------------------------------------------------------
{
	return m_BScalingMode;
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetRotationalHandle(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetRotationalHandle(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScalingHandlesOn()
//----------------------------------------------------------------------------
{
  for(int i=0; i<4; i++)
  {
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScalingHandlesOff()
//----------------------------------------------------------------------------
{
  for(int i=0; i<4; i++)
  {
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetCenterHandle(double x, double y, double z)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetCenterHandle(double *xyz)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetCenterHandleActor()
//----------------------------------------------------------------------------
{
	return m_CenterHandle;
}
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetRotationalHandleActor()
//----------------------------------------------------------------------------
{
	return m_RotationalHandle;
}
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetNorthScalingHandleActor()
//----------------------------------------------------------------------------
{
	return m_Handle[3];
}
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetSouthScalingHandleActor()
//----------------------------------------------------------------------------
{
	return m_Handle[0];
}
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetEastScalingHandleActor()
//----------------------------------------------------------------------------
{
	return m_Handle[1];
}
//----------------------------------------------------------------------------
vtkActor* albaOpMMLContourWidget::GetWestScalingHandleActor()
//----------------------------------------------------------------------------
{
	return m_Handle[2];
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetHandleRadius(float r)
//----------------------------------------------------------------------------
{
	m_HandleRadius = r;
}
//----------------------------------------------------------------------------
float albaOpMMLContourWidget::GetHandleRadius()
//----------------------------------------------------------------------------
{
	return m_HandleRadius;
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetRotationHandleVisibility()
//----------------------------------------------------------------------------
{/*
	// get z level
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::RotationHandleOn()
//----------------------------------------------------------------------------
{
	GetRotationalHandleActor()->VisibilityOn();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::RotationHandleOff()
//----------------------------------------------------------------------------
{
	GetRotationalHandleActor()->VisibilityOff();
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ComputeBounds(double *b)
//----------------------------------------------------------------------------
{
  m_M->GetContourActor()->GetBounds(b);
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ComputeCenter(double *c)
//----------------------------------------------------------------------------
{
  double b[6];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::UpdateScalingHandles()
//----------------------------------------------------------------------------
{
	// get z level
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::UpdateRotationHandle()
//----------------------------------------------------------------------------
{
	// get z level
	double z = m_M->GetCurrentZOfSyntheticScans();
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::UpdateWidgetTransform()
//----------------------------------------------------------------------------
{
	// get z level
	double z = m_M->GetCurrentZOfSyntheticScans();
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScaleNorth(double *p1, double *p2)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScaleSouth(double *p1, double *p2)
//----------------------------------------------------------------------------
{
  // get vector of motion
  float v[3];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScaleEast(double *p1, double *p2)
//----------------------------------------------------------------------------
{
  // get vector of motion
  float v[3];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::ScaleWest(double *p1, double *p2)
//----------------------------------------------------------------------------
{
  // get vector of motion
  float v[3];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::Place(double *p1, double *p2)
//----------------------------------------------------------------------------
{
   // get vector of motion
  double m[3];
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::Translate(double *p1, double *p2)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::Rotate(double *p1, double *p2)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int albaOpMMLContourWidget::GetNextOperationId()
//----------------------------------------------------------------------------
{
  return m_OperationID;
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetNextOperationId(int n)
//----------------------------------------------------------------------------
{
  m_OperationID = n;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetModel(albaOpMMLModelView *Model)
//----------------------------------------------------------------------------
{
	m_M = Model;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetPH(albaOpMMLParameterView *PH)
//----------------------------------------------------------------------------
{
	m_CH = PH;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetPV(albaOpMMLParameterView *PV)
//----------------------------------------------------------------------------
{
	m_CV = PV;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetTH(albaOpMMLParameterView *TH)
//----------------------------------------------------------------------------
{
	m_H = TH;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetTV(albaOpMMLParameterView *TV)
//----------------------------------------------------------------------------
{
	m_V = TV;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetRA(albaOpMMLParameterView *RA)
//----------------------------------------------------------------------------
{
	m_T = RA;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetSN(albaOpMMLParameterView *SN)
//----------------------------------------------------------------------------
{
	m_N = SN;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetSS(albaOpMMLParameterView *SS)
//----------------------------------------------------------------------------
{
	m_S = SS;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetSE(albaOpMMLParameterView *SE)
//----------------------------------------------------------------------------
{
	m_E = SE;
}
...
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetSW(albaOpMMLParameterView *SW)
//----------------------------------------------------------------------------
{
	m_W = SW;
}
//----------------------------------------------------------------------------
void albaOpMMLContourWidget::SetRepresentation(int representation)
//----------------------------------------------------------------------------
{
