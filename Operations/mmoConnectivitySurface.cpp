...
{  	
  ...
  if(!m_TestMode)
  {
	  ...
	  mafString labelX;
	  labelX.Append(wxString::Format(_("DimX:  %.2f"),(bounds[1]-bounds[0])/2));
	  m_Gui->Label(labelX);

	  mafString labelY;
	  labelY.Append(wxString::Format(_("DimY:  %.2f"),(bounds[3]-bounds[2])/2));
	  m_Gui->Label(labelY);

	  mafString labelZ;
	  labelZ.Append(wxString::Format(_("DimZ:  %.2f"),(bounds[5]-bounds[4])/2));
	  m_Gui->Label(labelZ);
  ...
  if(!m_TestMode)
  {
	...
	for(int region=0, deleteRegion = 0; region< regionNumbers; region++)
	{
      ...
      connectivityFilter->DeleteSpecifiedRegion(deleteRegion);
		...
		dimX = (bounds[1] - bounds[0])/2;
		dimY = (bounds[3] - bounds[2])/2;
		dimZ = (bounds[5] - bounds[4])/2;
