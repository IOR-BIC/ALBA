  '-----------------------------------------------------------------------------
  ' AQTime coverage profiler execution script
  '-----------------------------------------------------------------------------
  ...
  Set logFile = fso.CreateTextFile(".\build\bin\Debug\Coverage\XML\CoverageAllocationLog.txt", True)
  logFile.WriteLine("** Log file for AQTime coverage profiling: **")
  logFile.WriteLine("")
  logFile.Write("coverage profile started on ")
  ...
  For Each ExeFile In absoluteExePath.Files
    ...
    If Right(ExeFile, 4) = ".exe" Then 
      ...
      If canProfile = True Then
        ...
        If Not IntegrationManager.SelectProfiler("Coverage Profiler") Then 
          'MsgBox "The specified profiler was not found."
           Dim profilerMessage
           profilerMessage = "Cannot find the Coverage Profiler"
      ...
      If canProfile = True Then      
        ...
        Dim XMLFileName
        XMLSummaryFile = CStr(absoluteExePath) + "\Coverage\XML\CoverageSummary-" + CStr(exeFileName) + ".xml"
        XMLResultsFile = CStr(absoluteExePath) + "\Coverage\XML\CoverageResults-" + CStr(exeFileName)+".xml"
        ...
        REM Dim strLiveObjectsNumber
        REM strLiveObjectsNumber = GetLiveObjects(XMLSummaryFile)
        REM If strLiveObjectsNumber = "0" Then 
          REM ' MsgBox "no leaks detected"
        REM Else
          REM ' MsgBox CStr(exeFileName) + " has leaks! " + CStr(strLiveObjectsNumber) + " live objects detected"
          REM Dim message
          REM message = CStr(exeFileName) + " has leaks! " + CStr(strLiveObjectsNumber) + " live objects detected"
          REM logFile.WriteLine(CStr(message))
          REM ' increase leaked files number
          REM intLeakedFiles = intLeakedFiles + 1
          REM intTotalLeak = intTotalLeak + strLiveObjectsNumber
