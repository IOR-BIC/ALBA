/*=========================================================================
  ...
  Module:    $RCSfile: vtkALBAVolumeResample.cxx,v $
  Language:  C++
...
inline int albaVolumeResliceFloor(double x)
{
...
inline int albaVolumeResliceCeil(double x)
{
  return -albaVolumeResliceFloor(-x - 1.0) - 1;
}

inline int albaVolumeResliceRound(double x)
{
  return albaVolumeResliceFloor(x + 0.5);
}

static const int SamplingTableSize = 64000;
/*
void vtkALBAVolumeResample::myDBG(const char *msg,int index)
{
  cerr<<"myDBG: ";
  if (msg)
    cerr<<msg<<" ";
...
//----------------------------------------------------------------------------
void vtkALBAVolumeResample::SetVolumeAxisX(double axis[3])
{
...
//----------------------------------------------------------------------------
void vtkALBAVolumeResample::SetVolumeAxisY(double axis[3]) {
  if (vtkMath::Norm(axis) < 1.e-5f)
    return;
...
//----------------------------------------------------------------------------
void vtkALBAVolumeResample::PrepareVolume() {
...
void vtkALBAVolumeResample::RequestData(vtkInformation* request, vtkImageData *outputObject) 
{
	...
	{
		case VTK_CHAR:
			this->CreateImage((const char*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_UNSIGNED_CHAR:
			this->CreateImage((const unsigned char*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_SHORT:
			this->CreateImage((const short*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_UNSIGNED_SHORT:
			this->CreateImage((const unsigned short*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_INT:
			this->CreateImage((const int*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_UNSIGNED_INT:
			this->CreateImage((const int*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_FLOAT:
			this->CreateImage((const float*)inputPointer, outputScalars, outputObject);
			break;
		case VTK_DOUBLE:
			this->CreateImage((const double*)inputPointer, outputScalars, outputObject);
			break;
...
void vtkALBAVolumeResample::CreateImage(const InputDataType *input, vtkDataArray *outputScalars, vtkImageData *outputObject)
{
	...
	{
		case VTK_CHAR:
			this->CreateImage(input, (char*)outputPointer, outputObject);
			break;
		case VTK_UNSIGNED_CHAR:
			this->CreateImage(input, (unsigned char*)outputPointer, outputObject);
			break;
		case VTK_SHORT:
			this->CreateImage(input, (short*)outputPointer, outputObject);
			break;
		case VTK_UNSIGNED_SHORT:
			this->CreateImage(input, (unsigned short*)outputPointer, outputObject);
			break;
		case VTK_INT:
			this->CreateImage(input, (int*)outputPointer, outputObject);
			break;
		case VTK_UNSIGNED_INT:
			this->CreateImage(input, (int*)outputPointer, outputObject);
			break;
		case VTK_FLOAT:
			this->CreateImage(input, (float*)outputPointer, outputObject);
			break;
		case VTK_DOUBLE:
			this->CreateImage(input, (double*)outputPointer, outputObject);
			break;
...
//----------------------------------------------------------------------------
template<typename InputDataType, typename OutputDataType> void vtkALBAVolumeResample::CreateImage(const InputDataType *input, OutputDataType *output, vtkImageData *outputObject) 
{  
  ...
  for (int c = 0; c < 3; c++) {
    ...
    for (int i = 0, ti0 = 0; i <= lastIndex; i++) {
      //const int ti1 = albaVolumeResliceRound((i != lastIndex) ? (coords[i] - coords[0]) * coordToIndex : (SamplingTableSize - 1));
      const int ti1 = albaVolumeResliceFloor((i != lastIndex) ? (coords[i] - coords[0]) * coordToIndex : (SamplingTableSize));
      if (ti1 <= ti0)
        continue;
  ...
  for (int zi = 0; zi < zs; zi++) 
  {
    ...
    for (int yi = 0; yi < ys; yi++) 
    {
      ...
      for (int xi = 0; xi < xs; xi++, p[0] += xaxis[0], p[1] += xaxis[1], p[2] += xaxis[2], pixel += di/*, this->Num++*/) 
      {
        ...
        const u_int pi[3] = { albaVolumeResliceFloor(p[0]), albaVolumeResliceFloor(p[1]), albaVolumeResliceFloor(p[2])};
        //const u_int pi[3] = { albaVolumeResliceRound(p[0]), albaVolumeResliceRound(p[1]), albaVolumeResliceRound(p[2])};
...
//----------------------------------------------------------------------------
void vtkALBAVolumeResample::CalculateTextureCoordinates(const double point[3], const int size[2], const double spacing[2], double ts[2]) {
...
void vtkALBAVolumeResample::PrintSelf( ostream& os, vtkIndent indent )
{
  os << "-------------------------------------------------------" << std::endl;
  os << "vtkALBAVolumeResample PrintSelf:" << std::endl;
