...
#include "vtkALBAMultiResolutionActor.h"
...
//-----------------------------------------------------------------------------
vtkALBAMultiResolutionActor* vtkALBAMultiResolutionActor::New()
//-----------------------------------------------------------------------------
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkALBAMultiResolutionActor");
  if(ret)
    return (vtkALBAMultiResolutionActor*)ret;
  
  // If the factory was unable to create the object, then create it here.
  return new vtkALBAMultiResolutionActor;
}

//----------------------------------------------------------------------------
vtkALBAMultiResolutionActor::vtkALBAMultiResolutionActor()
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
  ...
  if (MultiThreader) 
  {
    // each thread adds a reference to vtkALBAMultiResolutionActor object, so it cannot be deleted
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::PrintSelf(ostream& os, vtkIndent indent) 
//-----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::Render(vtkRenderer *ren, vtkMapper *vtkNotUsed(m)) 
//-----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
vtkMapper *vtkALBAMultiResolutionActor::GetLastDrawnMapper() 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
vtkPolyData *vtkALBAMultiResolutionActor::GetLastDrawnPolyData() 
//----------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
int vtkALBAMultiResolutionActor::GetNumberOfResolutions() const 
//----------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
int vtkALBAMultiResolutionActor::GetTotalNumberOfResolutions() const 
//----------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
vtkMapper *vtkALBAMultiResolutionActor::GetMapper(int index) const 
//----------------------------------------------------------------------------
{
  if (index == 0)
    return this->Mapper;
...
//--------------------------------------------------------------------------
int vtkALBAMultiResolutionActor::GetUsage(int mapper) 
//----------------------------------------------------------------------------
{
  if (this->Mapper == NULL || this->Mapper->GetInput() == NULL || this->InputData != this->Mapper->GetInput() ||
...
//--------------------------------------------------------------------------
double vtkALBAMultiResolutionActor::GetRenderTime(int mapper) 
//----------------------------------------------------------------------------
{
  if (this->Mapper == NULL || this->Mapper->GetInput() == NULL || this->InputData != this->Mapper->GetInput() ||
      this->InputData->GetMTime() > (unsigned long)this->BuildTime || mapper < 0 || mapper > NumberOfMappers)
...
//--------------------------------------------------------------------------
int vtkALBAMultiResolutionActor::RenderOpaqueGeometry(vtkViewport *vp) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::ReleaseGraphicsResources(vtkWindow *renWin) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::CreateMultiResolutionModels() 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::DeleteMultiResolutionModels() 
//----------------------------------------------------------------------------
{
  // stop any threads
...
//----------------------------------------------------------------------------
{
  ...
  vtkALBAMultiResolutionActor* actor = (vtkALBAMultiResolutionActor*)info->UserData;
...
struct DecimateProgressStruct 
{
  ...
  vtkALBAMultiResolutionActor *actor;
};
...
void vtkALBAMultiResolutionActor::CreateDecimatedModels(bool parallel) 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::Modified() 
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
  vtkALBAMultiResolutionActor *a = vtkALBAMultiResolutionActor::SafeDownCast(prop);
  if ( a != NULL ) 
  {
...
//----------------------------------------------------------------------------
void vtkALBAMultiResolutionActor::StopAndDeleteThread() 
//----------------------------------------------------------------------------
{
