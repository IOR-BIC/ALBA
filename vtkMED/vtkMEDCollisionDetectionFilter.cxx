...
vtkMEDCollisionDetectionFilter::vtkMEDCollisionDetectionFilter()
{
...
void vtkMEDCollisionDetectionFilter::SetInput(int idx, vtkPolyData *input)
{
...
//----------------------------------------------------------------------------
vtkPolyData *vtkMEDCollisionDetectionFilter::GetInput(int idx)
{
  if (2 <= idx || idx < 0)
    {
    vtkErrorMacro(<< "Index " << idx 
...
void vtkMEDCollisionDetectionFilter::SetTransform(int i, vtkLinearTransform *transform)
{
  if (i > 1 || i < 0)
    {
    vtkErrorMacro(<< "Index " << i 
...
void vtkMEDCollisionDetectionFilter::SetMatrix(int i, vtkMatrix4x4 *matrix)
{
  if (i > 1 || i < 0)
    {
    vtkErrorMacro(<< "Index " << i 
...
vtkMatrix4x4* vtkMEDCollisionDetectionFilter::GetMatrix(int i)
{
  if (this->Transform[i]) 
    { 
...
static int ComputeCollisions(vtkOBBNode *nodeA, vtkOBBNode *nodeB, vtkMatrix4x4 *Xform, void *clientdata)
{
  ...
  vtkMEDCollisionDetectionFilter* self = reinterpret_cast<vtkMEDCollisionDetectionFilter *>( clientdata );
  ...
  if (self->GetCollisionMode() == vtkMEDCollisionDetectionFilter::VTK_ALL_CONTACTS)
    {
  ...
  if (self->GetCollisionMode() == vtkMEDCollisionDetectionFilter::VTK_FIRST_CONTACT) 
    {
    FirstContact = 1;
    }
...
void vtkMEDCollisionDetectionFilter::Execute()
{
  ...
  if (this->CollisionMode == vtkMEDCollisionDetectionFilter::VTK_ALL_CONTACTS)
...
// is an intersection.
int vtkMEDCollisionDetectionFilter::IntersectPolygonWithPolygon(int npts, double *pts, double bounds[6],
  ...
  for (i=0; i<npts; i++) 
    ...
    if ( (vtkPlane::IntersectWithLine(p1,p2,n2,pts2,t,x[Num])) == 1 ) 
      ...
      if ( (npts2==3
        ...
        {
        Num++;
        if (CollisionMode != vtkMEDCollisionDetectionFilter::VTK_ALL_CONTACTS ||
          Num == 2)
          {
          return 1;
          }
...
unsigned long vtkMEDCollisionDetectionFilter::GetMTime()
{
...
void vtkMEDCollisionDetectionFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
int vtkMEDCollisionDetectionFilter::GetNumberOfContacts()
{
