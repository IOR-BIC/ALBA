/*========================================================================= 
  ...
  Module: $RCSfile: mafPipeTensorField.cpp,v $ 
  Language: C++ 
...
//----------------------------------------------------------------------------
mafPipeTensorField::mafPipeTensorField() : mafPipe()
//----------------------------------------------------------------------------
{  
...
//----------------------------------------------------------------------------
void mafPipeTensorField::Create(mafSceneNode *n)
//----------------------------------------------------------------------------
{
  Superclass::Create(n); 
...
/*virtual*/ void mafPipeTensorField::ComputeDefaultParameters()
//------------------------------------------------------------------------
{
...
int mafPipeTensorField::GetFieldIndex(const char* szName, bool bTensors)
//------------------------------------------------------------------------
{
...
int mafPipeTensorField::GetNumberOfFields(bool bTensors)
//------------------------------------------------------------------------
{
  vtkPointData* pd = m_Vme->GetOutput()->GetVTKData()->GetPointData();
  if (pd == NULL)
...
const char* mafPipeTensorField::GetFieldName(int nIndex, bool bTensors)
//------------------------------------------------------------------------
{
  vtkPointData* pd = m_Vme->GetOutput()->GetVTKData()->GetPointData();
  if (pd == NULL)
...
void mafPipeTensorField::PopulateCombo(wxComboBox* combo, bool bTensors)
//------------------------------------------------------------------------
{
  vtkPointData* pd = m_Vme->GetOutput()->GetVTKData()->GetPointData();
  if (pd != NULL)
