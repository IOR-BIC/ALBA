...
mafCxxTypeMacro(mafPipeWrappedMeter);

//----------------------------------------------------------------------------
mafPipeWrappedMeter::mafPipeWrappedMeter() 
//----------------------------------------------------------------------------
{
  m_DataMapper        = NULL;
  m_DataActor         = NULL;
...
//----------------------------------------------------------------------------
void mafPipeWrappedMeter::Create(mafSceneNode *n/*, bool use_axes*/)
//----------------------------------------------------------------------------
{
  Superclass::Create(n);
  ...
  assert(m_Vme->IsA("mafVMEWrappedMeter"));
  m_WrappedMeterVME = mafVMEWrappedMeter::SafeDownCast(m_Vme);
  ...
  vtkNEW(m_DataMapper);
  if (m_WrappedMeterVME->GetMeterRepresentation() == mafVMEWrappedMeter::LINE_REPRESENTATION)
  ...
  if(m_WrappedMeterVME->GetMeterColorMode() == mafVMEWrappedMeter::RANGE_COLOR)
...
//    Caption->GetProperty()->SetColor(data->m_mat_gui->GetMaterial()->m_prop->GetColor());
 ...
 /* if(m_WrappedMeterVME->GetMeterMode() == mafVMEWrappedMeter::LINE_ANGLE)
    m_Caption->SetVisibility((m_WrappedMeterVME->GetAngle() != 0) && m_WrappedMeterVME->GetMeterAttributes()->m_LabelVisibility);
  else*/
    m_Caption->SetVisibility((m_WrappedMeterVME->GetDistance() >= 0 || m_WrappedMeterVME->GetMeterMeasureType() == mafVMEWrappedMeter::RELATIVE_MEASURE) && m_WrappedMeterVME->GetMeterAttributes()->m_LabelVisibility);
...
//----------------------------------------------------------------------------
mafGUI *mafPipeWrappedMeter::CreateGui()
//----------------------------------------------------------------------------
{
  ...
  m_MaterialButton->Enable(meter_attrib->m_ColorMode == mafVMEWrappedMeter::ONE_COLOR);
  m_Gui->Enable(ID_DISTANCE_RANGE,meter_attrib->m_ColorMode == mafVMEWrappedMeter::RANGE_COLOR);
  m_Gui->Enable(ID_TUBE_RADIUS, meter_attrib->m_Representation == mafVMEWrappedMeter::TUBE_REPRESENTATION);
  m_Gui->Enable(ID_TUBE_CAPPING, meter_attrib->m_Representation == mafVMEWrappedMeter::TUBE_REPRESENTATION);
	m_Gui->Divider();

  return m_Gui;
}
//----------------------------------------------------------------------------
void mafPipeWrappedMeter::OnEvent(mafEventBase *maf_event)
//----------------------------------------------------------------------------
{
  ...
  else if (maf_event->GetSender() == m_WrappedMeterVME)
  {
    ...
    else if(maf_event->GetId() == mafVMEWrappedMeter::LENGTH_THRESHOLD_EVENT) 
    {
    }
  }
}
//----------------------------------------------------------------------------
void mafPipeWrappedMeter::Select(bool sel)
//----------------------------------------------------------------------------
{
	m_Selected = sel;
...
//----------------------------------------------------------------------------
void mafPipeWrappedMeter::UpdateProperty(bool fromTag)
//----------------------------------------------------------------------------
{
  if (!m_DataMapper)
    return;
 ...
 // if(m_WrappedMeterVME->GetMeterMode() == mafVMEWrappedMeter::LINE_ANGLE) distance_value = m_WrappedMeterVME->GetAngle();
  ...
  if(color_mode == mafVMEWrappedMeter::RANGE_COLOR)
  {
    double *range;
    m_DataMapper->SetLookupTable(m_Lut);
    range = m_WrappedMeterVME->GetDistanceRange();
