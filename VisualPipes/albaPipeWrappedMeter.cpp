...
albaCxxTypeMacro(albaPipeWrappedMeter);

//----------------------------------------------------------------------------
albaPipeWrappedMeter::albaPipeWrappedMeter() 
//----------------------------------------------------------------------------
{
  m_DataMapper        = NULL;
  m_DataActor         = NULL;
...
//----------------------------------------------------------------------------
void albaPipeWrappedMeter::Create(albaSceneNode *n/*, bool use_axes*/)
//----------------------------------------------------------------------------
{
  Superclass::Create(n);
  ...
  assert(m_Vme->IsA("albaVMEWrappedMeter"));
  m_WrappedMeterVME = albaVMEWrappedMeter::SafeDownCast(m_Vme);
  ...
  if(m_WrappedMeterVME->GetMeterColorMode() == albaVMEWrappedMeter::RANGE_COLOR)
...
//    Caption->GetProperty()->SetColor(data->m_mat_gui->GetMaterial()->m_prop->GetColor());
  ...
  m_Caption->BorderOff();
  wxString dis;
  dis = albaString::Format("%.2f",m_WrappedMeterVME->GetDistance());
  m_Caption->SetCaption(dis.ToAscii());
 ...
 /* if(m_WrappedMeterVME->GetMeterMode() == albaVMEWrappedMeter::LINE_ANGLE)
    ...
    m_Caption->SetVisibility((m_WrappedMeterVME->GetDistance() >= 0 || m_WrappedMeterVME->GetMeterMeasureType() == albaVMEWrappedMeter::RELATIVE_MEASURE) && m_WrappedMeterVME->GetMeterAttributes()->m_LabelVisibility);
...
//----------------------------------------------------------------------------
albaGUI *albaPipeWrappedMeter::CreateGui()
//----------------------------------------------------------------------------
{
  ...
  m_MaterialButton->Enable(meter_attrib->m_ColorMode == albaVMEWrappedMeter::ONE_COLOR);
  m_Gui->Enable(ID_DISTANCE_RANGE,meter_attrib->m_ColorMode == albaVMEWrappedMeter::RANGE_COLOR);
  m_Gui->Enable(ID_TUBE_RADIUS, meter_attrib->m_Representation == albaVMEWrappedMeter::TUBE_REPRESENTATION);
  m_Gui->Enable(ID_TUBE_CAPPING, meter_attrib->m_Representation == albaVMEWrappedMeter::TUBE_REPRESENTATION);
	m_Gui->Divider();

  return m_Gui;
}
//----------------------------------------------------------------------------
void albaPipeWrappedMeter::OnEvent(albaEventBase *alba_event)
//----------------------------------------------------------------------------
{
  ...
  else if (alba_event->GetSender() == m_WrappedMeterVME)
  {
    ...
    else if(alba_event->GetId() == albaVMEWrappedMeter::LENGTH_THRESHOLD_EVENT) 
    {
    }
  }
}
//----------------------------------------------------------------------------
void albaPipeWrappedMeter::Select(bool sel)
//----------------------------------------------------------------------------
{
	m_Selected = sel;
...
//----------------------------------------------------------------------------
void albaPipeWrappedMeter::UpdateProperty(bool fromTag)
//----------------------------------------------------------------------------
{
  if (!m_DataMapper)
    return;
 ...
 // if(m_WrappedMeterVME->GetMeterMode() == albaVMEWrappedMeter::LINE_ANGLE) distance_value = m_WrappedMeterVME->GetAngle();
  ...
  if(color_mode == albaVMEWrappedMeter::RANGE_COLOR)
  {
    double *range;
    m_DataMapper->SetLookupTable(m_Lut);
    range = m_WrappedMeterVME->GetDistanceRange();
