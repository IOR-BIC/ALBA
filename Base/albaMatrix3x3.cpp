...
albaCxxTypeMacro(albaMatrix3x3)
...
//------------------------------------------------------------------------------
void albaMatrix3x3::Print (std::ostream& os, const int indent) const
//------------------------------------------------------------------------------
{
...
template<class T>
//----------------------------------------------------------------------------
inline void albaSwapVectors3(T v1[3], T v2[3])
{
  for (int i = 0; i < 3; i++)
  {
    T tmp = v1[i];
...
//----------------------------------------------------------------------------
void albaMatrix3x3::GetVersor(int axis, const albaMatrix3x3 &matrix, double versor[3])
//----------------------------------------------------------------------------
{
	if (0 <= axis && axis <= 2)
	{
...
//----------------------------------------------------------------------------
void albaMatrix3x3::Zero(double elements[9])
//----------------------------------------------------------------------------
{
  for (int i = 0; i < 9; i++)
  {
...
//----------------------------------------------------------------------------
void albaMatrix3x3::MultiplyVector(const float A[3][3], const float in[3], float out[3])
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void albaMatrix3x3::MultiplyVector(const double A[3][3], const double in[3], double out[3])
//----------------------------------------------------------------------------
{
  out[0] = A[0][0] * in[0] + A[0][1] * in[1] + A[0][2] * in[2];
  out[1] = A[1][0] * in[0] + A[1][1] * in[1] + A[1][2] * in[2];
...
//----------------------------------------------------------------------------
void albaMatrix3x3::Multiply(const double A[3][3], const double B[3][3],double C[3][3])
//----------------------------------------------------------------------------
{
  double D[3][3];
...
//----------------------------------------------------------------------------
void albaMatrix3x3::Transpose(const double A[3][3], double AT[3][3])
//----------------------------------------------------------------------------
{
  double tmp;
  tmp = A[1][0];
...
//----------------------------------------------------------------------------
void albaMatrix3x3::Invert(const double A[3][3], double AI[3][3])
//----------------------------------------------------------------------------
{
  int index[3];
  double tmp[3][3];
  ...
  albaMatrix3x3::LUFactor(AI,index);
  for (int i = 0; i < 3; i++)
  {
    double *x = tmp[i];
    x[0] = x[1] = x[2] = 0.0;
    x[i] = 1.0;
    albaMatrix3x3::LUSolve(AI,index,x);
  }
  for (int j = 0; j < 3; j++)
  {
    double *x = tmp[j];
...
void albaMatrix3x3::LUFactor(double A[3][3], int index[3])
//----------------------------------------------------------------------------
{
  int i,maxI;
  double tmp,largest;
...
void albaMatrix3x3::LUSolve(const double A[3][3], const int index[3], double x[3])
//----------------------------------------------------------------------------
{
  double sum;
...
//----------------------------------------------------------------------------
void albaMatrix3x3::Identity(double A[3][3])
//----------------------------------------------------------------------------
{
  for (int i = 0; i < 3; i++)
    {
...
#define ALBA_ROTATE(a,i,j,k,l) g=a[i][j];h=a[k][l];a[i][j]=g-s*(h+g*tau);\
        a[k][l]=h+s*(g-h*tau)

#define ALBA_MAX_ROTATIONS 20
...
// normalized.
int albaMatrix3x3::JacobiN(double **a, int n, double *w, double **v)
//----------------------------------------------------------------------------
{
  ...
  for (i=0; i<ALBA_MAX_ROTATIONS; i++) 
    {
    sm = 0.0;
    for (ip=0; ip<n-1; ip++) 
      {
    ...
    for (ip=0; ip<n-1; ip++) 
      ...
      for (iq=ip+1; iq<n; iq++) 
        ...
        && (fabs(w[iq])+g) == fabs(w[iq]))
          ...
          for (j = 0;j <= ip-1;j++) 
            {
            ALBA_ROTATE(a,j,ip,j,iq);
            }
          // ip and iq already shifted left by 1 unit
          for (j = ip+1;j <= iq-1;j++) 
            {
            ALBA_ROTATE(a,ip,j,j,iq);
            }
          // iq already shifted left by 1 unit
          for (j=iq+1; j<n; j++) 
            {
            ALBA_ROTATE(a,ip,j,iq,j);
            }
          for (j=0; j<n; j++) 
            {
            ALBA_ROTATE(v,j,ip,j,iq);
            }
          }
        }
      }
  ...
  if ( i >= ALBA_MAX_ROTATIONS )
  {
    albaWarningMacro("albaMatrix3x3::Jacobi: Error extracting eigenfunctions");
    return 0;
  }
...
//----------------------------------------------------------------------------
void albaMatrix3x3::QuaternionToMatrix(const double quat[4], double A[3][3])
//----------------------------------------------------------------------------
{
...
void albaMatrix3x3::MatrixToQuaternion(const double A[3][3], double quat[4])
//----------------------------------------------------------------------------
{
  double N[4][4];
  ...
  albaMatrix3x3::JacobiN(NTemp,4,eigenvalues,eigenvectorsTemp);
...
void albaMatrix3x3::Orthogonalize(const double A[3][3], double B[3][3])
//----------------------------------------------------------------------------
{
  int i;
  ...
  if (index[0] != 0) 
    {
    albaSwapVectors3(B[index[0]],B[0]);
  ...
  if (index[1] != 1)
    {
    albaSwapVectors3(B[index[1]],B[1]);
    }
  if (index[0] != 0)
    {
    albaSwapVectors3(B[index[0]],B[0]);
...
// axes respectively.
void albaMatrix3x3::Diagonalize(const double A[3][3], double w[3], double V[3][3])
{
  int i,j,k,maxI;
  double tmp, maxVal;
  ...
  albaMatrix3x3::JacobiN(ATemp,3,w,VTemp);
  
  // if all the eigenvalues are the same, return identity matrix
  if (w[0] == w[1] && w[0] == w[2])
    {
    albaMatrix3x3::Identity(V);
    return;
    }

  // transpose temporarily, it makes it easier to sort the eigenvectors
  albaMatrix3x3::Transpose(V,V);
  ...
  for (i = 0; i < 3; i++)
    ...
    if (w[(i+1)%3] == w[(i+2)%3]) // two eigenvalues are the same
      ...
      V[j][2] = 0.0;
      V[j][j] = 1.0;
      albaMatrix3x3::Cross(V[maxI],V[j],V[k]);
      albaMatrix3x3::Normalize(V[k]);
      albaMatrix3x3::Cross(V[k],V[maxI],V[j]);
  ...
  if (albaMatrix3x3::Determinant(V) < 0)
    {
    V[2][0] = -V[2][0];
    V[2][1] = -V[2][1];
    V[2][2] = -V[2][2];
...
void albaMatrix3x3::SingularValueDecomposition(const double A[3][3], 
...
//----------------------------------------------------------------------------
{
  ...
  double d = albaMatrix3x3::Determinant(B);
  if (d < 0)
  {
    for (i = 0; i < 3; i++)
    {
  ...
  albaMatrix3x3::Orthogonalize(B, U);
  albaMatrix3x3::Transpose(B, B);
  albaMatrix3x3::Multiply(B, U, VT);
  albaMatrix3x3::Diagonalize(VT, w, VT);
  albaMatrix3x3::Multiply(U, VT, U);
  albaMatrix3x3::Transpose(VT, VT);
