...
//----------------------------------------------------------------------------
mafView *mafViewSliceOnCurve::Copy(mafObserver *Listener, bool lightCopyEnabled)
//----------------------------------------------------------------------------
{  
...
//----------------------------------------------------------------------------
void mafViewSliceOnCurve::VmeShow(mafVME *vme, bool show)
//----------------------------------------------------------------------------
{
...
void mafViewSliceOnCurve::HideSameVMEs(mafView* view, mafVME* vme)
//------------------------------------------------------------------------
{
...
int mafViewSliceOnCurve::GetNodeStatus(mafVME *vme)
//----------------------------------------------------------------------------
{
  if (vme->GetOutput()->IsA("mafVMEOutputPolyline"))
  {
...
/*virtual*/ mafPipe* mafViewSliceOnCurve::GetNodePipe(mafVME *vme)
//------------------------------------------------------------------------
{
...
{	
  ...
  if (m_CurrentVolume)
  {
    ...
    p = mafPipeVolumeArbSlice::SafeDownCast(this->GetNodePipe(m_CurrentVolume));
...
/*virtual*/ void mafViewSliceOnCurve::LayoutSubView(int width, int height)
//------------------------------------------------------------------------
{
  int border = 2;
  int x_pos, y_pos;
...
/*virtual*/ void mafViewSliceOnCurve::CreateGizmo(mafVME* vme)
//------------------------------------------------------------------------
{
  assert(m_Gizmo == NULL);
  if (m_Gizmo != NULL)    
...
{	
  for (int i = 0; i < 2; i++)
  {
    ...
    {	
      ...
      vs->PlugVisualPipe("mafVMEVolumeGray", "mafPipeVolumeArbSlice", MUTEX);
      vs->PlugVisualPipe("mafVMELabeledVolume", "mafPipeVolumeArbSlice", MUTEX);
      vs->PlugVisualPipe("mafVMEVolumeRGB", "mafPipeVolumeArbSlice", MUTEX);
    }
  }
}
...
/*virtual*/ void mafViewSliceOnCurve::PlugSurfacePipe()
{
  for (int i = 0; i < 2; i++)
  {
    ...
    {	
      mafViewVTK* vs = ((mafViewVTK*)vws[SLICE_VIEW]);
      vs->PlugVisualPipe("mafVMESurface", "mafPipeSurfaceSlice");
      vs->PlugVisualPipe("mafVMESurfaceParametric", "mafPipeSurfaceSlice");
    }
  }
}
