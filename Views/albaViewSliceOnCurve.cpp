...
Module: $RCSfile: albaViewSliceOnCurve.cpp,v $ 
Language: C++ 
...
//----------------------------------------------------------------------------
albaCxxTypeMacro(albaViewSliceOnCurve);
//----------------------------------------------------------------------------
#include "albaMemDbg.h"


//3D volume pipes to be plugged
/*static*/ const albaViewSliceOnCurve::VPIPE_ENTRY albaViewSliceOnCurve::m_VolumePipes[] = 
{
  {"albaPipeVolumeDRR", "DRR"},
  {"albaPipeVolumeMIP", "MIP"},
  {"albaPipeVolumeVR", "VR"},
  {"albaPipeIsosurface", "ISO"},
  {NULL, NULL},  
};

//layout names
/*static*/ const char* albaViewSliceOnCurve::m_LayoutNames[] = 
{
  "one row",
  "Mps horz", "Mps vert", 
  "Smp horz", "Smp vert",
...
//----------------------------------------------------------------------------
albaViewSliceOnCurve::albaViewSliceOnCurve(wxString label) : albaViewCompound(label, 1, 3)
//----------------------------------------------------------------------------
{  
  m_LayoutConfiguration = albaViewSliceOnCurve::LAYOUT_MPS_VERT; 
...
//----------------------------------------------------------------------------
albaViewSliceOnCurve::~albaViewSliceOnCurve()
//----------------------------------------------------------------------------
{  
  if (m_Gizmo != NULL)
  {
...
//----------------------------------------------------------------------------
albaView *albaViewSliceOnCurve::Copy(albaObserver *Listener, bool lightCopyEnabled)
//----------------------------------------------------------------------------
{  
  m_LightCopyEnabled = lightCopyEnabled;
  albaViewSliceOnCurve *v = this->NewInstance(); 
  v->m_Label = m_Label;
...
//----------------------------------------------------------------------------
void albaViewSliceOnCurve::VmeShow(albaVME *vme, bool show)
//----------------------------------------------------------------------------
{
...
void albaViewSliceOnCurve::HideSameVMEs(albaView* view, albaVME* vme)
//------------------------------------------------------------------------
{
...
int albaViewSliceOnCurve::GetNodeStatus(albaVME *vme)
//----------------------------------------------------------------------------
{
  if (vme->GetOutput()->IsA("albaVMEOutputPolyline"))
  {
...
/*virtual*/ void albaViewSliceOnCurve::Create()
//-------------------------------------------------------------------------
{
  Superclass::Create();
...
//-------------------------------------------------------------------------
albaGUI* albaViewSliceOnCurve::CreateGui()
//-------------------------------------------------------------------------
{	
  assert(m_Gui == NULL);
...
/*virtual*/ void albaViewSliceOnCurve::LayoutSubView(int width, int height)
//------------------------------------------------------------------------
{
  int border = 2;
  int x_pos, y_pos;
...
//----------------------------------------------------------------------------
void albaViewSliceOnCurve::OnEvent(albaEventBase *alba_event)
//----------------------------------------------------------------------------
{
...
/*virtual*/ void albaViewSliceOnCurve::CreateGizmo(albaVME* vme)
//------------------------------------------------------------------------
{
  assert(m_Gizmo == NULL);
  if (m_Gizmo != NULL)    
  ...
  {
    albaLogMessage("Unsupported VME for albaViewSliceOnCurve::CreateGizmo");
    return;
  }     
...
/*virtual*/ void albaViewSliceOnCurve::DestroyGizmo()
//------------------------------------------------------------------------
{
  if (m_Gizmo != NULL)
  {
...
/*virtual*/ void albaViewSliceOnCurve::SetSlice(double* Origin, double* Normal)
//------------------------------------------------------------------------
{
...
/*virtual*/ void albaViewSliceOnCurve::OnGizmoMoved()
//------------------------------------------------------------------------
{
...
void albaViewSliceOnCurve::SetSlicePosition(double abscisa, vtkIdType branchId)
{
  if (m_Gizmo == NULL)
...
//----------------------------------------------------------------------------
/*virtual*/ void albaViewSliceOnCurve::PackageView()
//----------------------------------------------------------------------------
{
  //create child views
...
/*virtual*/ void albaViewSliceOnCurve::PlugVolumePipe()
{	
  for (int i = 0; i < 2; i++)
  {
    ...
    if (MAIN_VIEW < vws.size())
    {
      ...
      v->PlugVisualPipe("albaVMEVolumeGray", m_VolumePipes[m_VolumePipeConfiguration].szClassName, MUTEX);	
      v->PlugVisualPipe("albaVMELabeledVolume", m_VolumePipes[m_VolumePipeConfiguration].szClassName, MUTEX);
      v->PlugVisualPipe("albaVMEVolumeRGB", m_VolumePipes[m_VolumePipeConfiguration].szClassName, MUTEX);
    }
...
/*virtual*/ void albaViewSliceOnCurve::PlugSurfacePipe()
{
  for (int i = 0; i < 2; i++)
  {
    std::vector< albaView* >& vws = i == 0 ? m_PluggedChildViewList : m_ChildViewList;
...
/*virtual*/ void albaViewSliceOnCurve::PlugMeshPipe()
{
  for (int i = 0; i < 2; i++)
  {
    std::vector< albaView* >& vws = i == 0 ? m_PluggedChildViewList : m_ChildViewList;
...
/*virtual*/ void albaViewSliceOnCurve::PlugPolylinePipe()
//------------------------------------------------------------------------
{
  //Plug polyline pipe
  for (int i = 0; i < 2; i++)
...
/*virtual*/ void albaViewSliceOnCurve::ChangeVolumePipe(const char* pipename)
{
...
/*virtual*/ int albaViewSliceOnCurve::GetNumberOfVolumePipes()
//------------------------------------------------------------------------
{
  int nRet = 0;
...
/*virtual*/ void albaViewSliceOnCurve::UpdateGizmoStatusText(const char* szText)
//------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
void albaViewSliceOnCurve::Print(std::ostream& os, const int tabs)// const
//-------------------------------------------------------------------------
{
  albaIndent indent(tabs);
  os << indent << "albaViewSliceOnCurve" << '\t' << this << std::endl;
...
/*virtual*/ void albaViewSliceOnCurve::OnShowPolylineInMainView()
//------------------------------------------------------------------------
{
...
/*virtual*/ void albaViewSliceOnCurve::OnShowGizmoCoords()
//------------------------------------------------------------------------
{
