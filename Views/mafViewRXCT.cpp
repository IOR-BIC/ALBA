...
void mafViewRXCT::VmeShow(mafVME *vme, bool show)
{
  ...
  if (vme->GetOutput()->IsA("mafVMEOutputVolume"))
  {
    if (show && !isVmeShowed)
    {
      ...
      mafVMEOutputVolume *volumeOutput = mafVMEOutputVolume::SafeDownCast(vme->GetOutput());
      ...
      for(int i=0; i<CT_CHILD_VIEWS_NUMBER; i++)
      {
        ...
        p = mafPipeVolumeOrthoSlice::SafeDownCast(((mafViewSlice *)((mafViewCompound *)m_ChildViewList[CT_COMPOUND_VIEW])->GetSubView(i))->GetNodePipe(vme));
  ...
  else if (vme->IsMAFType(mafVMESurface)||vme->IsMAFType(mafVMESurfaceParametric))
  {
    // showing a surface with the volume present already
    if (show && m_CurrentVolume)
    {
      // create the slice in every CT views
      mafVME *vmeSelected = this->GetSceneGraph()->GetSelectedVme();
      ((mafViewCompound *)m_ChildViewList[CT_COMPOUND_VIEW])->VmeShow(vme,show);
      for(int j=0;j<CT_CHILD_VIEWS_NUMBER;j++)
      {
        int i=0;
        while (j!=m_Sort[i]) i++;
    ...
    else if (!show)
    {
      ...
      for(int i=0; i<CT_CHILD_VIEWS_NUMBER; i++)
      {
        ((mafViewSlice *)((mafViewCompound *)m_ChildViewList[CT_COMPOUND_VIEW])->GetSubView(i))->UpdateSurfacesList(vme);
      }
    }//else if(show)
  }
  else
...
void mafViewRXCT::OnEventSortSlices()
{
  ...
  if(node->GetOutput()->IsA("mafVMEOutputVolume"))
      ...
      case ID_TRILINEAR_INTERPOLATION:
        {
          if (m_CurrentVolume)
          {
            ...
            mafPipeVolumeOrthoSlice *p = NULL;
            p = mafPipeVolumeOrthoSlice::SafeDownCast(((mafViewSlice *)((mafViewCompound *)m_ChildViewList[RX_SIDE_VIEW]))->GetNodePipe(m_CurrentVolume));
            if (p)
            {
              p->SetInterpolation(m_TrilinearInterpolationOn);
            }
            p = mafPipeVolumeOrthoSlice::SafeDownCast(((mafViewSlice *)((mafViewCompound *)m_ChildViewList[RX_FRONT_VIEW]))->GetNodePipe(m_CurrentVolume));
            if (p)
            {
              p->SetInterpolation(m_TrilinearInterpolationOn);
            }
...
mafGUI* mafViewRXCT::CreateGui()
{
  ...
  if (m_CurrentVolume)
  {
    ...
    {
      mafPipeVolumeOrthoSlice *p = NULL;
      p = mafPipeVolumeOrthoSlice::SafeDownCast(((mafViewSlice *)m_ChildViewList[i])->GetNodePipe(m_CurrentVolume));
      if (p)
      {
        p->SetInterpolation(m_TrilinearInterpolationOn);
      }
