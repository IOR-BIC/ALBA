/*=========================================================================
Program:   ALBA
Module:    $RCSfile: albaVMEStent.h,v $
Language:  C++
...
#ifndef __albaVMEStent_h 
#define __albaVMEStent_h

#include "albaDefines.h"  //very important for ALBA_EXPORT
...
typedef vtkALBAStentModelSource::StentConfigurationType        enumStCfgType;
typedef vtkALBAStentModelSource::LinkConnectionType            enumLinkConType;
typedef vtkALBAStentModelSource::LinkOrientationType           enumLinkOrtType;
typedef std::vector<Strut>::const_iterator    StrutIterator;
typedef itk::vtkALBADeformableSimplexMeshFilter<SimplexMeshType,SimplexMeshType> DeformFilterType;
...
//-----------------------------------------------------------------------------
class ALBA_EXPORT albaVMEStent : public albaVMEGeneric
{
public:
  /// Risk categories
  enum{
  ...
  albaTypeMacro(albaVMEStent, albaVMEGeneric);

  albaVMEStent(); ///< Constructor
  ~albaVMEStent(); ///< Destructor

  int DeepCopy(albaVME *a) ; ///< Deep copy

  albaString GetVisualPipe() {return albaString("albaPipePolyline");}; ///< Get pipe name
  albaVMEOutputPolyline *GetPolylineOutput(); ///< return the right type of output 
  ...
  void OnEvent(albaEventBase *alba_event); ///< Event handler
  ...
  /// Get the vessel vme
  albaVME* GetVesselVME() {return m_VesselVME ;}

  /// Get the centerline vme
  albaVME* GetCenterLineVME() {return m_CenterLineVME ;}

  /// Set the deployed polydata vme.
  void SetDeployedPolydataVME(albaVME* inputNode) ;
  ...
  void SetStentLink(const char *link_name, albaVME *ns);
...
private:
  ...
  albaVME* FindOrSelectCenterLineVME(albaVME* inputNode) ;

  /// Find or select the associated vessel surface vme. \n
  /// Adds tag if not already present. \n
  /// Does nothing and returns true if vme already found. \n
  /// Returns NULL if failed.
  albaVME* FindOrSelectVesselVME(albaVME* inputNode) ;

  /// Find the tagged center line vme. \n
  /// Returns NULL if failed.
  albaVME* FindTaggedCenterLineVME(albaVME* inputNode) ;

  /// Find the tagged vessel surface vme. \n
  /// Returns NULL if failed.
  albaVME* FindTaggedVesselVME(albaVME* inputNode) ;

  void SetVesselCenterLine(albaVME *node);
  void SetVesselSurface(albaVME *node);
  ...
  /// Returns NULL if failed.
  albaVME* FindNodeWithId(albaID id) ;
  ...
  // stent
  vtkALBAStentModelSource *m_StentSource;
  ...
  albaVMEPolyline *m_DeployedPolydataVME ; 
