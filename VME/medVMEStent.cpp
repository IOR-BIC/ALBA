  ...
  m_IterationsPerStep_Default(2), m_NumberOfSteps_Default(300)
{
  ...
  m_DeformFilter = DeformFilterType::New() ; // NB smart itk pointer - don't delete
}
  ...
  //-------------------------------------------------------------------------
{
  ...
  if (mafEvent *e = mafEvent::SafeDownCast(maf_event))
  {
    ...
    case ID_CENTERLINE:
      ...
      break;
    case CHANGE_VIEW:{
      DisplayCatheter(); //display or remove catheter from output
      m_EventSource->InvokeEvent(this, VME_OUTPUT_DATA_UPDATE);
      ForwardUpEvent(&mafEvent(this,CAMERA_UPDATE));
                     }	  
                     break;
...
void medVMEStent::DisplayStentExpandByStep(mafTimeStamp t)
{
  int n1 = GetNumberOfTimeStamps() ;
  int n2 = GetNumberOfLocalTimeStamps() ;
  ...
  //at lease catheter needs update, moveCatheter its self will decide if update needed
  if(m_ShowCatheter){
    moveCatheter(t*2);
  }
  UpdateViewAfterDeformation();
...
void medVMEStent::PreComputeStentPointsBySteps( int steps){
  ...
  for (int i=0; i<steps; i++)
  {
    ...
    m_DeformFilter->SetCurIterationNum(i*m_IterationsPerStep); // used to synchronize expansion with catheter
...
void medVMEStent::createTestVesselPolydata(vtkPolyData  *centerLine){
  ...
  nav->PrintCells(sheath->GetOutput(), thing) ;
  thing << "\n\n\n\n" ;
  nav->PrintCells(tubePolys, thing) ;
  thing.close() ;
  nav->Delete() ;
...
{
  vtkPolyData *copyOfCenterLine = vtkPolyData::New() ;
  copyOfCenterLine->DeepCopy(m_Centerline);

  int numPtsOnCenterLine = copyOfCenterLine->GetNumberOfPoints();
  ...
  *-----------*/
  int speed = 4;
  int m = (currentIter/speed+100 < numPtsOnCenterLine-1) ? (currentIter/speed+100) : numPtsOnCenterLine-1;
