...
//-------------------------------------------------------------------------
{
  ...
  // fill in the VME's with data and pose matrices to create a beautiful animation
  int i;
  for (i=0;i<100;i++)
  {
    trans.Identity();
    ...
    if ((i%10)==0)
    {
      ...
      mafTransform tmp;
      tmp.SetMatrix(test_vtitle_pose[i/10]);
      tmp.Concatenate(trans.GetMatrix(),PRE_MULTIPLY);
      tmp.SetTimeStamp(i*.5+25);
      test_sphere_abspose[i/10]=tmp.GetMatrix();
    }
  ...
  for (i=0;i<10;i++)
  {
    ...
    double x[3],y[3];
    pdata->GetPoints()->GetPoint(pdata->GetNumberOfPoints()/2,x);
    test_sphere[i]->GetOutput()->GetPoints()->GetPoint(pdata->GetNumberOfPoints()/2,y);
  ...
  /*for (i=0;i<10;i++)
  {
    // is abs pose matrix generated by abs matrix pipe correct?
    mafMatrix *abs_pose=vsphere->GetOutput()->GetAbsMatrix();
    MAF_TEST(abs_pose!=NULL);
    MAF_TEST(*abs_pose==test_sphere_abspose[i]);
  }*/
