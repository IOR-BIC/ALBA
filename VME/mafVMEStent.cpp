/*=========================================================================
Program:   MAF2
Module:    $RCSfile: mafVMEStent.cpp,v $
Language:  C++
...
//------------------------------------------------------------------------------
mafCxxTypeMacro(mafVMEStent);
//------------------------------------------------------------------------------

//------------------------------------------------------------------------------
// Constructor
//------------------------------------------------------------------------------
mafVMEStent::mafVMEStent()
  : m_StentStartPosId(0), 
...
//-------------------------------------------------------------------------
//-------------------------------------------------------------------------
mafVMEOutputPolyline *mafVMEStent::GetPolylineOutput()
{
...
// Event handler
//-------------------------------------------------------------------------
void mafVMEStent::OnEvent(mafEventBase *maf_event)
{
...
{ 
  ...
  mafVMEStent *vmeStent = mafVMEStent::SafeDownCast(a);
...
//-----------------------------------------------------------------------
int mafVMEStent::InternalStore(mafStorageElement *node)
{  
...
//-----------------------------------------------------------------------
int mafVMEStent::InternalRestore(mafStorageElement *node)
{
...
//------------------------------------------------------------------------------
void mafVMEStent::UpdateStentPolydataFromSimplex()
{
  if (m_SimplexMeshModified){
    if(m_StentSource->getInphaseShort()==1)
...
// Update stent polydata from simplex
//------------------------------------------------------------------------------
void mafVMEStent::UpdateStentPolydataFromSimplex_old()
{
  if (m_SimplexMeshModified){
    //----------------------------------------
...
// Update stent polydata from simplex
//------------------------------------------------------------------------------
void mafVMEStent::UpdateStentPolydataFromSimplex_Simple()
{
  if (m_SimplexMeshModified){
    //----------------------------------------
    // copy the simplex vertices to vtkPoints
...
// Update stent polydata from simplex
//------------------------------------------------------------------------------
void mafVMEStent::UpdateStentPolydataFromSimplex_Abbott()
{
  if (m_SimplexMeshModified){
    //----------------------------------------
    // copy the simplex vertices to vtkPoints
...
//------------------------------------------------------------------------------
void mafVMEStent::CalculateMidPointsFromPairOfStruts(const double strutEndPts[4][3], double midPts[4][3], bool isLinkPoint, double strutLinkPts[2][3]) const
{
...
//------------------------------------------------------------------------------
void mafVMEStent::UpdateStentPolydataFromSimplex_ViewAsSimplex()
{
  if (m_SimplexMeshModified){
    vtkPoints* vpoints = vtkPoints::New();
    vpoints->SetNumberOfPoints(40000);		
...
//-----------------------------------------------------------------------
void mafVMEStent::SetVesselCenterLine(vtkPolyData *line){
  if (line == NULL)
    return ;
...
//------------------------------------------------------------------------------
void mafVMEStent::SetVesselSurface(vtkPolyData *surface){
  if(surface){
...
// Calc strut angle
//------------------------------------------------------------------------------
void mafVMEStent::CalcStrutAngle()
{
...
// Calc strut length
//------------------------------------------------------------------------------
void mafVMEStent::CalcStrutLength()
{
...
//------------------------------------------------------------------------------
vtkPolyData* mafVMEStent::GetStentPolyData()
{ 
  InternalUpdate();
...
// update data and return polydata                                                                    
//------------------------------------------------------------------------------
vtkPolyData* mafVMEStent::GetSimplexPolyData()
{ 
  InternalUpdate();
...
//-------------------------------------------------------------------------
void mafVMEStent::SetStentStartPos(int startId) 
{
...
//-------------------------------------------------------------------------
void mafVMEStent::CreateExtrapolatedLine(vtkPolyData* lineIn, vtkPolyData* lineOut, double extrapFactor)
{
...
//-------------------------------------------------------------------------
void mafVMEStent::CreateTruncatedLine(vtkPolyData* lineIn, vtkPolyData* lineOut, int id0, int id1)
{
  lineOut->Initialize() ;
...
//-------------------------------------------------------------------------
void mafVMEStent::CreateLongVesselCenterLine()
{
...
//-------------------------------------------------------------------------
void mafVMEStent::CreateStentCenterLine()
{
...
//-------------------------------------------------------------------------
void mafVMEStent::GetValidPointIds(vtkPolyData* pd, vtkIdList* idList) const
{
...
/// Needed because the stent contains unused and undefined points.
//-------------------------------------------------------------------------
int mafVMEStent::GetHighestValidPointIndex(vtkPolyData *pd) const
{
...
//-------------------------------------------------------------------------
int mafVMEStent::CalcStentLengthVerts()
{
...
//-------------------------------------------------------------------------
double mafVMEStent::CalcStentLengthMM()
{
...
//-------------------------------------------------------------------------
double mafVMEStent::CalcStentDiameterMM()
{
  vtkIdList* validPts = vtkIdList::New() ;
  GetValidPointIds(m_StentPolyData, validPts) ;
...
//-------------------------------------------------------------------------
void mafVMEStent::CalcArcLengthsOfPoints(vtkPolyData *pd, double* arcLengths) 
{
...
//-------------------------------------------------------------------------
double mafVMEStent::CalcArclengthFromIdPosition(vtkPolyData *pd, int id, double lambda) 
{
  int n = pd->GetPoints()->GetNumberOfPoints() ;
  assert((id >= 0) && (id < n)) ;
...
// This assumes that the pd is a monotonic curve of points.
//-------------------------------------------------------------------------
void mafVMEStent::CalcIdPositionFromArcLength(vtkPolyData *pd, double arclen, int& id, double& lambda) 
{
  int n = pd->GetPoints()->GetNumberOfPoints() ;
...
// This assumes that the pd is a monotonic curve of points.
//-------------------------------------------------------------------------
void mafVMEStent::CalcCoordsFromIdPosition(vtkPolyData *pd, int id, double lambda, double* x) 
{
  int n = pd->GetPoints()->GetNumberOfPoints() ;
  assert((id >= 0) && (id < n)) ;
...
//-------------------------------------------------------------------------
void mafVMEStent::FindNearestPointOnCenterLine(double* p0, vtkPolyData *pd, int& id, double& distSq) 
{
  int n = pd->GetPoints()->GetNumberOfPoints() ;
...
//-------------------------------------------------------------------------
void mafVMEStent::FindNearestPointOnCenterLine(double* p0, vtkPolyData *pd, int& id, double& lambda, double& distSq) 
{
  int n = pd->GetPoints()->GetNumberOfPoints() ;
...
//-------------------------------------------------------------------------
void mafVMEStent::PartialInitDefFilterFromStentModel()
{
  // set the input mesh
...
//-------------------------------------------------------------------------
bool mafVMEStent::IsSimplexMeshModified()
{
...
//-------------------------------------------------------------------------
void mafVMEStent::DoDeformationStep() 
{
...
//-------------------------------------------------------------------------
void mafVMEStent::CrimpStent(double crimpedDiameter)
{
...
//-------------------------------------------------------------------------
void mafVMEStent::SetStentPolyData(vtkPolyData* pd)
{
...
//-------------------------------------------------------------------------
int mafVMEStent::GetNumberOfUnits()
{
  int n ;
...
// Get length of unit
//-------------------------------------------------------------------------
double mafVMEStent::GetLengthOfUnit()
{
  double len ;
