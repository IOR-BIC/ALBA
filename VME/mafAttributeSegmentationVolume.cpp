...
//----------------------------------------------------------------------------
mafCxxTypeMacro(mafAttributeSegmentationVolume)
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
mafAttributeSegmentationVolume::mafAttributeSegmentationVolume()
//----------------------------------------------------------------------------
{  
  m_Name = "SegmentationVolumeData";

  m_AutomaticSegmentationThresholdModality = mafVMESegmentationVolume::GLOBAL;
...
//-------------------------------------------------------------------------
void mafAttributeSegmentationVolume::DeepCopy(const mafAttribute *a)
//-------------------------------------------------------------------------
{ 
  if (!a->IsA("mafAttributeSegmentationVolume"))
  {
  	return;
  }
  ...
  //////////////////////////////////////////////////////////////////////////
 	m_AutomaticSegmentationThresholdModality = ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationThresholdModality();
  m_UseDoubleThreshold= ((mafAttributeSegmentationVolume*)a)->GetDoubleThresholdModality();
  m_AutomaticSegmentationGlobalThreshold = ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationGlobalThreshold();
  m_AutomaticSegmentationGlobalUpperThreshold = ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationGlobalUpperThreshold();
  for (int i=0;i<((mafAttributeSegmentationVolume*)a)->GetNumberOfRanges();i++)
  {
    int startSlice,endSlice;
    double threshold;
    ((mafAttributeSegmentationVolume*)a)->GetRange(i,startSlice,endSlice,threshold);
    this->AddRange(startSlice,endSlice,threshold);
  }
  //////////////////////////////////////////////////////////////////////////
  m_RegionGrowingLowerThreshold = ((mafAttributeSegmentationVolume*)a)->GetRegionGrowingLowerThreshold();
  m_RegionGrowingUpperThreshold = ((mafAttributeSegmentationVolume*)a)->GetRegionGrowingUpperThreshold();
  for (int i=0;i<((mafAttributeSegmentationVolume*)a)->GetNumberOfSeeds();i++)
  {
    int seed[3];
    ((mafAttributeSegmentationVolume*)a)->GetSeed(i,seed);
    this->AddSeed(seed);
  }
  //////////////////////////////////////////////////////////////////////////
}
//----------------------------------------------------------------------------
bool mafAttributeSegmentationVolume::Equals(const mafAttribute *a)
//----------------------------------------------------------------------------
{
  if (Superclass::Equals(a))
  {
    if ( (m_AutomaticSegmentationThresholdModality != ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationThresholdModality()) ||
         (m_UseDoubleThreshold != ((mafAttributeSegmentationVolume*)a)->GetDoubleThresholdModality()) ||
         (m_AutomaticSegmentationGlobalThreshold != ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationGlobalThreshold()) ||
         (m_AutomaticSegmentationGlobalUpperThreshold != ((mafAttributeSegmentationVolume*)a)->GetAutomaticSegmentationGlobalUpperThreshold() && m_UseDoubleThreshold) ||
         (m_AutomaticSegmentationRanges.size() != ((mafAttributeSegmentationVolume*)a)->GetNumberOfRanges()) ||
         (m_RegionGrowingLowerThreshold != ((mafAttributeSegmentationVolume*)a)->GetRegionGrowingLowerThreshold()) ||
         (m_RegionGrowingUpperThreshold != ((mafAttributeSegmentationVolume*)a)->GetRegionGrowingUpperThreshold()) ||
         (m_RegionGrowingSeeds.size() != ((mafAttributeSegmentationVolume*)a)->GetNumberOfSeeds()) )
    {
      return false;
    }
...
//-----------------------------------------------------------------------
int mafAttributeSegmentationVolume::InternalStore(mafStorageElement *parent)
//-----------------------------------------------------------------------
{  
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::InternalRestore(mafStorageElement *node)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::AddRange(int startSlice,int endSlice,double threshold, double upperThreshold)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::GetRange(int index,int &startSlice, int &endSlice, double &threshold)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::GetRange(int index,int &startSlice, int &endSlice, double &threshold, double &upperThreshold)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::UpdateRange(int index,int startSlice, int endSlice, double threshold, double upperThreshold)
//----------------------------------------------------------------------------
{
  //on addrange you push back threshold in m_AutomaticSegmentationThresholds and m_AutomaticSegmentationUpperThresholds
  //if you are in single threshold modality m_AutomaticSegmentationUpperThresholds will be ignored
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::RemoveAllRanges()
//----------------------------------------------------------------------------
{
  for (int i=0;i<m_AutomaticSegmentationRanges.size();i++)
  {
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::RemoveAllSeeds()
//----------------------------------------------------------------------------
{
  for (int i=0;i<m_RegionGrowingSeeds.size();i++)
  {
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::DeleteSeed(int index)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::DeleteRange(int index)
//----------------------------------------------------------------------------
{
  int nRanges = m_AutomaticSegmentationRanges.size();
  if (index<0 || index>(nRanges-1))
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::AddSeed(int seed[3])
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
int mafAttributeSegmentationVolume::GetSeed(int index,int seed[3])
//----------------------------------------------------------------------------
{
  int nSeeds = m_RegionGrowingSeeds.size();
  if (index<0 || index>(nSeeds-1))
...
//-----------------------------------------------------------------------
void mafAttributeSegmentationVolume::Print(std::ostream& os, const int tabs) const
//-----------------------------------------------------------------------
{
