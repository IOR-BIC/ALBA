...
//----------------------------------------------------------------------------
{
  ...
  else 
  {
    ...
    albaLogMessage("Wizard Error: unable to select VME, path:\"%s\" base:\"%s\"",m_VmeSelect.char_str(),m_SelectedVME->GetName());
    Abort();
  ...
  if (m_RequiredView != "")
  {
    ...
    for(int i=0;i<m_VmeShow.size();i++)
    {
      ...
      albaVME *toShow=m_SelectedVME->GetByPath(m_VmeShow[i].char_str());
      if (toShow != NULL)
