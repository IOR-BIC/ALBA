...
albaLicenceManager::licenceStatuses albaLicenceManager::GetCurrentMode()
{
	...
	if (!RegKey.Exists())
	{
		...
		sprintf(tmp, "%s-code:%lu", m_AppName.char_str(), secs);
		...
		if(created)
			RegKey.SetValue("LocalKey", cryptedCode.char_str());
...
void albaLicenceManager::AddTimeLicence(wxDateTime expireDate)
{
	...
	wxRegKey RegKey(regKeyStr);
	RegKey.Create();
	RegKey.SetValue("Registered", cryptedDate.char_str());
}
...
void albaLicenceManager::AddBinaryLicence()
{
	...
	wxRegKey RegKey(regKeyStr);
	RegKey.Create();
	RegKey.SetValue("Registered", criptedReg.char_str());
}
...
albaLicenceManager::addLicenceStatuses albaLicenceManager::CheckCreateLicence(wxString registrationString)
{
	...
	decriptedKey = DecryptStr(localKey.char_str());
	...
	if (strncmp(decriptedKey.char_str(), decripRegString.char_str(), locKeyLen))
	{
		return WRONG_LICENCE;
	}
	...
	else //BINARY_LICENCE
	{
		...
		if (strcmp(regString.char_str(), "REGISTERED"))
			return WRONG_LICENCE;
...
wxString albaLicenceManager::EncryptStr(wxString plainStr)
{
	...
	if (m_CryptKey == "")
		albaDefaultEncryptFromMemory(plainStr.char_str(), encStr);
	else
		albaEncryptFromMemory(plainStr.char_str(), encStr, m_CryptKey.char_str());
...
wxString albaLicenceManager::DecryptStr(wxString plainStr)
{
	...
	if (m_CryptKey == "")
		albaDefaultDecryptInMemory(plainStr.char_str(), decStr);
	else
		albaDecryptInMemory(plainStr.char_str(), decStr, m_CryptKey.char_str());
...
albaLicenceManager::CreateNewLicenceStatuses albaLicenceManager::CreateNewTimeLicence(wxString RegCode, wxDateTime expirationDate, wxString &newLicence)
{
	...
	char tmp[255];
	sprintf(tmp, "%s-code:", m_AppName.char_str());
	...
	if (strncmp(tmp, baseRegCode.char_str(), baseRegLen))
		return WRONG_BASE_STRING;

	unsigned long secsInMil;
	sscanf(secsInMillennium.char_str(), "%ud", &secsInMil);
	...
	if (strcmp(fullCheckStr, decriptedRegCode.char_str()))
		return WRONG_FULL_STRING;

	char licence[255];
	sprintf(licence, "%s|%s", decriptedRegCode.char_str(), expirationDate.FormatISODate().char_str());
...
albaLicenceManager::CreateNewLicenceStatuses albaLicenceManager::CreateNewBinaryLicence(wxString RegCode, wxString &newLicence)
{
	...
	char licence[255];
	sprintf(licence, "%s|REGISTERED", decriptedRegCode.char_str());
...
void albaLicenceManager::ShowRegistrationDialog()
{
	...
	if (m_RegistrationDialog == NULL)
	{
		...
		if (wxFileExists(imgPath))
		{
			...
			m_PreviewImage->LoadFile(imgPath.char_str(), wxBITMAP_TYPE_ANY);
