...
#ifndef __mafWizard_H__
#define __mafWizard_H__
...
EXPORT_STL_VECTOR(MAF_EXPORT,mafWizardBlock *);
#endif


/**
  Class Name: mafWizard.
  Class for wizard creation each wizard must extend this block
*/
class MAF_EXPORT mafWizard : mafObserver
{
public:
  ...
  /** Default constructor */
  mafWizard(const wxString &label, const wxString &name="");
...
protected:
  ...
  void AddBlock(mafWizardBlock *block);
...
private:
  ...
  mafWizardBlock *GetBlockByName(const char *name);
  
  std::vector <mafWizardBlock *> m_Blocks;
  mafWizardBlock *m_CurrentBlock; 
  wxString        m_MenuPath;
