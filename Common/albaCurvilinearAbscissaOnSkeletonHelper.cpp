...
void albaCurvilinearAbscissaOnSkeletonHelper::MoveOnSkeleton( albaEvent *mouseEvent )
{
  ...
  albaTransform::GetPosition(incomingMatrix, inputMoveAbsVector);

  if (DEBUG_MODE)
  {
    albaLogMessage("Move WC Vector:") ;
    albaCurvilinearAbscissaOnSkeletonHelper::LogVector3(inputMoveAbsVector);
  }
...
void albaCurvilinearAbscissaOnSkeletonHelper::LogVector3( double *vector , const char *logMessage /*= NULL*/ )
{
...
void albaCurvilinearAbscissaOnSkeletonHelper::LogPoint( double *point, const char *logMessage )
{
  std::ostringstream stringStream;
  if (logMessage) stringStream << logMessage << std::endl;
...
bool albaCurvilinearAbscissaOnSkeletonHelper::IsBifurcationVertex( albaPolylineGraph *inPG, int inVertexID )
{
...
void albaCurvilinearAbscissaOnSkeletonHelper::FindTargetBranchAfterBifurcation( vtkIdType bifurcationVertexId, double moveAbsVector[3], 
...
{	
  ...
  vtkALBASmartPointer<vtkIdList> radiatingPointsIdList;
  ...
  vtkALBASmartPointer<vtkPoints> neighborsLocalCoordinates ;
  vtkALBASmartPointer<vtkPoints> localRadiatingVersors;
  ...
  if (DEBUG_MODE)
  {
    albaLogMessage("neighbors local coords and local versors:");
  }
  ...
  { 
    ...
    albaTransform::BuildVector(bifurcationPoint, localPoint, localVector);
  ...
  { 
    ...
    double currentProjection = albaTransform::ProjectVectorOnAxis(moveAbsVector, currentBifurcationVersor );
...
albaCurvilinearAbscissaOnSkeletonHelper::albaCurvilinearAbscissaOnSkeletonHelper( albaVME *inputVME, albaObserver *listener, bool testMode)
{
  m_InputVME = inputVME;
  ...
  m_ConstraintPolylineGraph = new albaPolylineGraph();
...
void albaCurvilinearAbscissaOnSkeletonHelper::SetConstraintPolylineGraph( albaVMEPolylineGraph* constraintPolylineGraph )
{
  ...
  {
    std::ostringstream stringStream;
    stringStream << "Checking for albaVMEPolylineGraph " << constraintPolylineGraph->GetName()
...
int albaCurvilinearAbscissaOnSkeletonHelper::SetCurvilinearAbscissa( vtkIdType branchId, double s )
{ 
...
void albaCurvilinearAbscissaOnSkeletonHelper::GetAbsPose( albaVMEPolylineGraph *inputConstrainVMEGraph, vtkIdType inBranchId, double s, albaMatrix &moverOutputAbsPose )
{

  std::ostringstream stringStream;

  const albaPolylineGraph::Branch *currentBranch = m_ConstraintPolylineGraph->GetConstBranchPtr(inBranchId);
  ...
  albaTransform::SetVersor(0, normal1_x_normal2, localGizmoPose);
  albaTransform::SetVersor(1, normal1, localGizmoPose);
  albaTransform::SetVersor(2, normal2, localGizmoPose);
  localGizmoPose.SetTimeStamp(-1);
  albaTransform::SetPosition(localGizmoPose, pOut);

  // get absolute pose for the constrain
  albaMatrix constrainAbsPose; // input
  constrainAbsPose = inputConstrainVMEGraph->GetAbsMatrixPipe()->GetMatrix();

  albaSmartPointer<albaTransform> trans; // output
...
void albaCurvilinearAbscissaOnSkeletonHelper::ComputeLocalPointPositionBetweenVerticesForSkeletonBranch( double distP0s, int idP0, int idP1, double pOut[3] )
{
  ...
  double p0p1[3] = {0,0,0};
  albaTransform::BuildVector(p0,p1,p0p1);
  vtkMath::Normalize(p0p1);

  double p0s[3] = {0,0,0};
  albaTransform::BuildVector(distP0s, p0p1, p0s, albaRefSys::GLOBAL);

  albaTransform::AddVectors(p0,p0s,pOut);

}

void albaCurvilinearAbscissaOnSkeletonHelper::FindPerpendicularVersorsToSegment( int idP0, int idP1, double viewUp[3], double normal[3] )
{
  double p0[3] = {0,0,0};
  double p1[3] = {0,0,0};
  ...
  albaTransform::FindPerpendicularVersors(p0p1, viewUp, normal) ;
  
}

double albaCurvilinearAbscissaOnSkeletonHelper::CheckS( vtkIdType inputBranchId, double inS )
{
...
void albaCurvilinearAbscissaOnSkeletonHelper::MoveOnSkeletonInternal( vtkIdType inBranchId, double inS, double inMoveAbsVector[3], vtkIdType &outputBranch, double &outputS, albaMatrix &outputGizmoAbsMatrix )
{
  ...
  albaTransform::ProjectVectorOnAxis(inMoveAbsVector, p0p1);
  ...
  {
    albaLogMessage("passed upper vertex!!! ################################################### ");
  ...
  {
    albaLogMessage("passed lower vertex!!! ################################################### ");
  ...
  {
    albaLogMessage("no vertex passed while dragging");
  ...
  if (isBifurcationVertex)
  {
    ...
    albaTransform::BuildVector(bifurcationPoint,p0, bifurcationPoint_LastSId_Versor);

    vtkMath::Normalize(bifurcationPoint_LastSId_Versor);

    double bifurcationPoint_LastSId_Vector[3]
    = {0,0,0};
    albaTransform::BuildVector(abs(dist_S_P1), bifurcationPoint_LastSId_Versor, bifurcationPoint_LastSId_Vector, albaRefSys::GLOBAL);
    ...
    albaTransform::BuildVector(bifurcationPoint,newEndVertex, bifurcationPoint_newEndVertex_Versor);
    vtkMath::Normalize(bifurcationPoint_newEndVertex_Versor);


    outputS = albaTransform::ProjectVectorOnAxis(outputMoveVector, bifurcationPoint_newEndVertex_Versor);
    ...
    {
      albaLogMessage("going toward beginning of branch-------->BEG-------->BEG-------->");

      outputS = startBranch_bifurcationPoint_Length - outputS;
    }
    else if(bifurcationPoindID  <  newEndVertexId)
    {
      albaLogMessage("going toward end of branch-------->END-------->END-------->");
      outputS = startBranch_bifurcationPoint_Length + outputS;
    }
...
void albaCurvilinearAbscissaOnSkeletonHelper::FindBoundaryVertices( vtkIdType inputSkeletonBranchId, double inS, int &outIdMin, int &outIdMax, double &outSFromIdMin )
{
  ...
  const albaPolylineGraph::Branch *currentBranch = m_ConstraintPolylineGraph->GetConstBranchPtr(inputSkeletonBranchId);
  if (DEBUG_MODE)
  {
    std::ostringstream stringStream;
  ...
  vtkALBASmartPointer<vtkIdList> branchPointsIdList;
...
void albaCurvilinearAbscissaOnSkeletonHelper::CreateGui()
{
...
void albaCurvilinearAbscissaOnSkeletonHelper::OnEvent(albaEventBase *alba_event)
{
...
void albaCurvilinearAbscissaOnSkeletonHelper::EnableWidgets( bool enable )
{
