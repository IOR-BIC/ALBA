    ...
    private static void CalculatePercentage(string resultDirXML)
    {
      ...
      try
      {
        ...
        foreach (string d in dirList)
        {
          ...
          string[] files = (string[])Array.CreateInstance(typeof(String), filesHeader.Length + filesImplem_cpp.Length + filesImplem_cxx.Length);
          filesHeader.CopyTo(files, 0);
          filesImplem_cpp.CopyTo(files, filesHeader.Length);
          filesImplem_cxx.CopyTo(files, filesHeader.Length + filesImplem_cpp.Length);
          Array.Sort(files);
