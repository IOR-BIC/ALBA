...
vtkCxxRevisionMacro(vtkVolumeSlicer, "$Revision: 1.13 $");
  ...
  for (int i = 0; i < this->GetNumberOfOutputs(); i++) 
  {
    if (vtkImageData::SafeDownCast(this->GetOutput(i))) 
    {
      ...
      { // select spacing
        ...
        this->PrepareVolume();
        const double d = -(this->GlobalPlaneAxisZ[0] * this->GlobalPlaneOrigin[0] + this->GlobalPlaneAxisZ[1] * this->GlobalPlaneOrigin[1] + this->GlobalPlaneAxisZ[2] * this->GlobalPlaneOrigin[2]);
...
//----------------------------------------------------------------------------
{
  ...
  for (i = 1; i < numberOfPoints; i++) 
  {
		...
		for (int j = 1; j < numberOfPoints; j++) 
    {
			...
			double norm = vtkMath::Norm(angleVector) * (vtkMath::Dot(angleVector, (double*)this->GlobalPlaneAxisZ) > 0. ? -1. : 1.);
