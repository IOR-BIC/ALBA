/*=========================================================================
  ...
  Module:    vtkMAFCollisionDetectionFilter.cxx
  Language:  C++
  RCS:   $Id: vtkMAFCollisionDetectionFilter.cxx,v 1.1.2.2 2012-02-22 12:53:16 ior02 Exp $
...
void vtkMAFCollisionDetectionFilter::SetInput(int idx, vtkPolyData *input)
{
...
//----------------------------------------------------------------------------
vtkPolyData *vtkMAFCollisionDetectionFilter::GetInput(int idx)
{
  if (2 <= idx || idx < 0)
    {
    vtkErrorMacro(<< "Index " << idx 
...
vtkIdTypeArray *vtkMAFCollisionDetectionFilter::GetContactCells(int i)
{
  if (2 <= i || i < 0)
    {
...
void vtkMAFCollisionDetectionFilter::SetTransform(int i, vtkLinearTransform *transform)
{
  if (i > 1 || i < 0)
    {
    vtkErrorMacro(<< "Index " << i 
...
void vtkMAFCollisionDetectionFilter::SetMatrix(int i, vtkMatrix4x4 *matrix)
{
  if (i > 1 || i < 0)
    {
    vtkErrorMacro(<< "Index " << i 
...
vtkMatrix4x4* vtkMAFCollisionDetectionFilter::GetMatrix(int i)
{
  if (this->Transform[i]) 
    { 
...
static int ComputeCollisions(vtkOBBNode *nodeA, vtkOBBNode *nodeB, vtkMatrix4x4 *Xform, void *clientdata)
{
  ...
  vtkMAFCollisionDetectionFilter* self = reinterpret_cast<vtkMAFCollisionDetectionFilter *>( clientdata );
  ...
  if (self->GetCollisionMode() == vtkMAFCollisionDetectionFilter::VTK_ALL_CONTACTS)
    {
  ...
  if (self->GetCollisionMode() == vtkMAFCollisionDetectionFilter::VTK_FIRST_CONTACT) 
    {
    FirstContact = 1;
    }
...
void vtkMAFCollisionDetectionFilter::Execute()
{
  ...
  if (this->CollisionMode == vtkMAFCollisionDetectionFilter::VTK_ALL_CONTACTS)
...
// is an intersection.
int vtkMAFCollisionDetectionFilter::IntersectPolygonWithPolygon(int npts, double *pts, double bounds[6],
  ...
  for (i=0; i<npts; i++) 
    ...
    if ( (vtkPlane::IntersectWithLine(p1,p2,n2,pts2,t,x[Num])) == 1 ) 
      ...
      if ( (npts2==3
        ...
        {
        Num++;
        if (CollisionMode != vtkMAFCollisionDetectionFilter::VTK_ALL_CONTACTS ||
          Num == 2)
          {
          return 1;
          }
...
unsigned long vtkMAFCollisionDetectionFilter::GetMTime()
{
...
void vtkMAFCollisionDetectionFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
int vtkMAFCollisionDetectionFilter::GetNumberOfContacts()
{
